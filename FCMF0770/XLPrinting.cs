...
namespace FCMF0770
{
    public class XLPrinting
    {
        ...
        public void HeaderWrite(object pYEAR_DATE, object pMONTH_DATE)
        {
            ...
            try
            {
                ...
                //YEAR                
                vString = string.Format("{0}", pYEAR_DATE);
                                
                mPrinting.XLSetCell(6, 23, vString);

                //SUB_YEAR
                mPrinting.XLActiveSheet(mSourceSheet3); //셀에 문자를 넣기 위해 쉬트 선택
                mPrinting.XLSetCell(3, 43, vString);

                //MONTH                                                
                vString = string.Format("{0}", pMONTH_DATE);
                
                mPrinting.XLSetCell(3, 37, vString);
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISDataAdapter pData)
        {
            ...
            mCopy_EndCol = 45;      // 복사될 Column 최대값
            mCopy_EndRow = 34;      // 복사될 Row 최대값

            mPrintingLastRow = 34;  //최종 인쇄 라인.
            mCurrentRow = 9;
