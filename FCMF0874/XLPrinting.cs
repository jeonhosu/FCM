...
namespace FCMF0826
{
    public class XLPrinting
    {
        ...
        private int mPrintingLineSTART = 11;  //Line
        ...
        private void SetArray1(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn, out int[] pXLColumn)
        {
            ...
            pGDColumn[0] = pGrid.GetColumnToIndex("DATE_YEAR"); //공급일자_년
            pGDColumn[1] = pGrid.GetColumnToIndex("DATE_MM"); //공급일자_월
            pGDColumn[2] = pGrid.GetColumnToIndex("DATE_DD"); //공급일자_일
            pGDColumn[3] = pGrid.GetColumnToIndex("SUPPLY_AMT"); //공급금액
            pGDColumn[4] = pGrid.GetColumnToIndex("CORP_NAME"); //업체 상호
            pGDColumn[5] = pGrid.GetColumnToIndex("VAT_NUMBER"); //사업자등록번호
            pGDColumn[6] = pGrid.GetColumnToIndex("LC_NO"); //내국신용장번호
            ...
            pXLColumn[3] = 9;  //공급금액
            pXLColumn[4] = 18; //업체 상호
            pXLColumn[5] = 32; //사업자등록번호
            pXLColumn[6] = 40; //내국신용장번호
        }
        ...
        private void XLHeader(InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_TARIFF_SPEC)
        {
            int vCountRow = 0;
            int vXLine = 0;
            int vXLColumn = 0;
            ...
            try
            {
                vCountRow = p_adapter_PRINT_TARIFF_SPEC.OraSelectData.Rows.Count;
                ...
                if (vCountRow > 0)
                {
                    ...
                    //성명(대표자)
                    vObject = p_adapter_PRINT_TARIFF_SPEC.OraSelectData.Rows[0]["PRESIDENT_NAME"];
                    vXLine = 5;
                    vXLColumn = 11;
                    ...
                    //상호(법인명)
                    vObject = p_adapter_PRINT_TARIFF_SPEC.OraSelectData.Rows[0]["CORP_NAME"];
                    vXLine = 6;
                    vXLColumn = 11;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //사업장소재지
                    vObject = p_adapter_PRINT_TARIFF_SPEC.OraSelectData.Rows[0]["LOCATION"];
                    vXLine = 7;
                    vXLColumn = 11;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //사업자등록번호
                    vObject = p_adapter_PRINT_TARIFF_SPEC.OraSelectData.Rows[0]["VAT_NUMBER"];
                    vXLine = 5;
                    vXLColumn = 38;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //업태
                    vObject = p_adapter_PRINT_TARIFF_SPEC.OraSelectData.Rows[0]["BUSINESS_ITEM"];
                    vXLine = 6;
                    vXLColumn = 38;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //종목
                    vObject = p_adapter_PRINT_TARIFF_SPEC.OraSelectData.Rows[0]["BUSINESS_TYPE"];
                    vXLine = 7;
                    vXLColumn = 38;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx p_grid_TARIFF_SPEC, InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_TARIFF_SPEC)
        {
            mPageNumber = 0;
            ...
            try
            {
                int vTotal1ROW = p_grid_TARIFF_SPEC.RowCount;

                #region ----- Header/SUM Write ----

                XLHeader(p_adapter_PRINT_TARIFF_SPEC);
                ...
                if (vTotal1ROW > 0)
                {
                    ...
                    SetArray1(p_grid_TARIFF_SPEC, out vGDColumn, out vXLColumn);
                    ...
                    for (int vRow1 = 0; vRow1 < vTotal1ROW; vRow1++)
                    {
                        ...
                        vPrintingLine = XLLine(p_grid_TARIFF_SPEC, vRow1, vPrintingLine, vGDColumn, vXLColumn);
        ...
        {
            int vPrintingLineEND = mCopyLineSUM - 2; //1~31: mCopyLineSUM=32에서 내용이 출력되는 행이 30 까지 이므로, 2을 빼면 된다
