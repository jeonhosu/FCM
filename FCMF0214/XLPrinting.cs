...
namespace FCMF0214
{
    public class XLPrinting
    {
        ...
        private int mPrintingLastRow = 67;  //실제 데이터 인쇄 최종 라인.

        private int mCurrentRow = 49;        //실제 인쇄되는 row 위치.
        ...
        {// 실제 호출되는 부분.
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    ...
                    vPageRowCount = mCurrentRow - 3;    //첫장에 대해서는 시작row부터 체크.
                    ...
                    foreach (System.Data.DataRow vRow in pLine.CurrentRows)
                    {
                        ...
                        else
                        {
                            ...
                            if (mIsNewPage == true)
                            {
                                mCurrentRow = mCurrentRow + (mCopy_EndRow - (mPrintingLastRow + 3) + mDefaultPageRow);  // 여러장 인쇄시 해당 페이지의 시작되는 위치.
