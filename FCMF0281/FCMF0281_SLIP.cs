...
namespace FCMF0281
{
    public partial class FCMF0281_SLIP : Office2007Form
    {
        #region ----- Variables -----
        ...
        ISGridAdvEx mGrid = new ISGridAdvEx();
        int mCURRENT_ROW = 0;
        string mStatus = "";
        ...
        public FCMF0281_SLIP()
        {
            InitializeComponent();
        }

        public FCMF0281_SLIP(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        public FCMF0281_SLIP(Form pMainForm, ISAppInterface pAppInterface, string pStatus, InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, int pRow)
        {
            InitializeComponent();
            isAppInterfaceAdv1.AppInterface = pAppInterface;
        ...
        {
            mStatus = "NON-QUERY";
            object vSLIP_HEADER_ID = mGrid.GetCellValue(mCURRENT_ROW, mGrid.GetColumnToIndex("SLIP_IF_HEADER_ID"));
            if (iString.ISNull(vSLIP_HEADER_ID) != string.Empty)
            {
                mStatus = "QUERY";
                
                idaSLIP_HEADER.SetSelectParamValue("W_HEADER_ID", vSLIP_HEADER_ID);
                try
                {
                    idaSLIP_HEADER.Fill();
                }
        ...
        private void Init_Budget_Dept()
        {
            ...
            {
                BUDGET_DEPT_NAME_L.EditValue = mGrid.GetCellValue("BUDGET_DEPT_NAME");
                BUDGET_DEPT_CODE_L.EditValue = mGrid.GetCellValue("BUDGET_DEPT_CODE"); ;
                BUDGET_DEPT_ID_L.EditValue = mGrid.GetCellValue("BUDGET_DEPT_ID");
            }
        }
        ...
        //        (itbSLIP.SelectedIndex == 1 && iConv.ISNull(HEADER_ID.EditValue) != string.Empty))
        //    {
        ...
        //        {
        //            //-------------------------------------------------------------------------------------
        //            xlPrinting.OpenFileNameExcel = "FCMF0206_001.xls";
        //            //-------------------------------------------------------------------------------------
        ...
        private void Insert_Silp_Header()
        {
            if (Check_SlipHeader_Added() == true)
            {
                return;
        ...
        private void FCMF0281_SLIP_Load(object sender, EventArgs e)
        {           
            // 회계장부 정보 설정.
            GetAccountBook();
        ...
        private void FCMF0281_SLIP_Shown(object sender, EventArgs e)
        {
            if (mStatus == "QUERY")
            {
                Search_DB();
            }
            else if (mStatus == "INSERT")
            {
                Insert_Silp_Header();
            }
        }
