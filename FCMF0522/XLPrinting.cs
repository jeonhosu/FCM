...
namespace FCMF0522
{
    public class XLPrinting
    {
        ...
        public void HeaderWrite(object pACCOUNT_CODE, object pACCOUNT_DESC, object pGL_DATE, object pGROUPING_OPTION,
                                string pTerritory, InfoSummit.Win.ControlAdv.ISGridAdvEx pGRID)
        {// 헤더 인쇄.
            int vXLine = 0;
            int vXLColumn = 0;
            ...
            try
            {
                ...
                vXLine = 1;
                vXLColumn = 1;
                mPrinting.XLSetCell(vXLine, vXLColumn, string.Format("({0}){1}", pACCOUNT_CODE, pACCOUNT_DESC));

                // 내역서
                vXLine = 1;
                vXLColumn = 4;
                mPrinting.XLSetCell(vXLine, vXLColumn, pGROUPING_OPTION);

                for (int c = 1; c < vTotalCol; c++)
                {// 프롬프트 표시.
                    ...
                    if (vVisible_YN == "1")
                    {
                        ...
                        mPrinting.XL_LineDraw_Right(mPromptRow, vCurrentCol, vCurrentCol, 1);   //프롬프트 라인 그리기.
                    }
                }
                //라인 그리기 시작.
                mPrinting.XL_LineDraw_TopBottom(mPromptRow, 1, vCurrentCol, 2);
                mPrinting.XL_LineDraw_Left(mPromptRow, 1, 1, 2);
                mPrinting.XL_LineDraw_Right(mPromptRow, vCurrentCol, vCurrentCol, 2);
                //라인 그리기 종료.

                //기준일자
                vXLine = 3;
                vXLColumn = vCurrentCol;
        ...
        {// 실제 호출되는 부분.
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    ...
                    {//Row
                        vSUMMARY_FLAG = iString.ISNull(pGRID.GetCellValue(r, vIDX_SUMMARY_FLAG), "N");
                        for (int c = 1; c < vTotalCol; c++)
                        {//Col
                            vVisible_YN = iString.ISNull(pGRID.GridAdvExColElement[c].Visible, "0");
                            if (vVisible_YN == "1")
                            {
                                ...
                                if (iString.ISNull(vColumnType) == "NumberEdit")
                                {
                                    try
                                    {
                                        ...
                                        else if (iString.ISNumtoZero(vDecimalDigit) > 0)
                                        {
                                            vPrintValue = string.Format("   {0:###,###,###,###,###,###,###,###,###.0000}", vNumberValue);
                                        }
                                        else
                                        {
                            ...
                            mPrinting.XL_LineDraw_Right(mCurrentRow, vCurrentCol, vCurrentCol, 1);   //세로라인 그리기.
                        ...
                        //라인 그리기 시작.
                        mPrinting.XL_LineDraw_Left(mCurrentRow, 1, 1, 2);
                        mPrinting.XL_LineDraw_Right(mCurrentRow, vCurrentCol, vCurrentCol, 2);

                        //해당 라인이 통화합계일 경우 실선으로 표시.
                        if (vPrint_Flag == false && vSUMMARY_FLAG == "CT")
                        {
                            vPrint_Flag = true;
                            mPrinting.XL_LineDraw_Top(mCurrentRow, 1, vCurrentCol, 2);
                        }
                        if (r == vTotalRow - 1)
                        {
                            mPrinting.XL_LineDraw_TopBottom(mCurrentRow, 1, vCurrentCol, 2);
                            //mPrinting.XL_LineDraw_Bottom(mCurrentRow, 1, vCurrentCol, 2);
                            mPrinting.XLColumnAutoFit(1, 1, mCurrentRow, vCurrentCol);
                        }
                        //라인 그리기 종료.
                        vCurrentCol = 0;
