...
namespace FCMF0889
{
    public class XLPrinting
    {
        ...
        private void SetArray1(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn, out int[] pXLColumn)
        {
            ...
            pGDColumn[3] = pGrid.GetColumnToIndex("ITEM_QTY");          //수량
            pGDColumn[4] = pGrid.GetColumnToIndex("ITEM_AMOUNT");       //금액(원)
            pGDColumn[5] = pGrid.GetColumnToIndex("DEEMED_VAT_AMOUNT"); //의제매입세액
        ...
        private int XLLine_2(System.Data.DataRow pRow, int pXLine)
        {
            ...
            try
            {
                ...
                //의제매입세액
                vXLColumnIndex = 40;
                vObject = pRow["DEEMED_VAT_AMOUNT"];
                IsConvert = IsConvertString(vObject, out vConvertString);
                if (IsConvert == true)
                {
                    vConvertString = string.Format("{0}", vConvertString);
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx pIGR_SUM_COPPER_ETC
                            , InfoSummit.Win.ControlAdv.ISDataAdapter pIDA_PRINT_COPPER_ETC
                            , InfoSummit.Win.ControlAdv.ISDataAdapter pIDA_PRINT_COPPER_ETC_TITLE)
        {
            mPageNumber = 0;
            ...
            try
            {
                int vTotal1ROW = pIGR_SUM_COPPER_ETC.RowCount;
                int vTotal2ROW = pIDA_PRINT_COPPER_ETC.CurrentRows.Count;

                #region ----- Header/SUM Write ----

                XLHeader(pIDA_PRINT_COPPER_ETC_TITLE);
                ...
                if (vTotal1ROW > 0)
                {
                    ...
                    SetArray1(pIGR_SUM_COPPER_ETC, out vGDColumn, out vXLColumn);
                    ...
                    for (int vRow1 = 0; vRow1 < vTotal1ROW; vRow1++)
                    {
                        ...
                        vPrintingLine = XLLine_1(pIGR_SUM_COPPER_ETC, vRow1, vPrintingLine, vGDColumn, vXLColumn);
                    }
                }
                ...
                if (vTotal2ROW > 0)
                {
                    ...
                    vPrintingLine = vPrintingLine + 4; // 17 = 13 + 4
                    foreach (System.Data.DataRow vRow in pIDA_PRINT_COPPER_ETC.CurrentRows)
                    {
                        vCountROW1 = vCountROW1 + 1;
                        vMessage = string.Format("Print Data Line : {0}/{1}", vCountROW1, vTotal2ROW);
                        mAppInterface.OnAppMessageEvent(vMessage);
                        System.Windows.Forms.Application.DoEvents();
        ...
        {
            int vPrintingLineEND = mCopyLineSUM - 7; //1~37: mCopyLineSUM=38에서 내용이 출력되는 행이 31 까지 이므로, 7을 빼면 된다
            if (mPageNumber != 1)
            {
                //첫 페이지외 모든 페이지
