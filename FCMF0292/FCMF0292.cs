...
namespace FCMF0292
{
    public partial class FCMF0292 : Office2007Form
    {
        ...
        private void GetAccountBook()
        {
            ...
            //적요 복사 여부//
            IDC_SLIP_REMARK_COPY_P.SetCommandParamValue("W_ASSEMBLY_ID", "FCMF0292");
        ...
        private void Search_DB()
        {
            ...
            else
            {
                ...
                IDA_BL_SLIP_HEADER_LIST.Refillable = true;
                IDA_BL_SLIP_HEADER_LIST.OraSelectData.AcceptChanges();

                string vGL_NUM = iString.ISNull(igrSLIP_LIST.GetCellValue("GL_NUM"));
                int vCOL_IDX = igrSLIP_LIST.GetColumnToIndex("GL_NUM");
                IDA_BL_SLIP_HEADER_LIST.Fill();
        ...
        private void XLPrinting(string pOutput_Type)
        {
                ...
                (itbSLIP.SelectedIndex == 1 && iString.ISNull(H_SLIP_HEADER_ID.EditValue) != string.Empty))
            {
                ...
                {
                    //-------------------------------------------------------------------------------------
                    xlPrinting.OpenFileNameExcel = "FCMF0292_001.xlsx";
                    //-------------------------------------------------------------------------------------
        ...
        private void XLPrinting_BSK(string pOutput_Type)
        {
                ...
                (itbSLIP.SelectedIndex == 1 && iString.ISNull(H_SLIP_HEADER_ID.EditValue) != string.Empty))
            {
                ...
                {
                    //-------------------------------------------------------------------------------------
                    xlPrinting.OpenFileNameExcel = "FCMF0292_011.xlsx";
                    //-------------------------------------------------------------------------------------
                    ...
                    if (isOpen == true)
                    {
                        ...
                        else
                        {
                            ...
                            vObject = igrSLIP_LIST.GetCellValue("HEADER_ID");
                        }
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    if (IDA_BL_SLIP_HEADER_LIST.IsFocused)
                    {
                        IDA_BL_SLIP_HEADER_LIST.Update();
                    }
                    else
                    {
                        ACCOUNT_CODE.Focus();
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Cancel)
                {
                    ...
                    if (IDA_BL_SLIP_HEADER_LIST.IsFocused)
                    {
                        IDA_BL_SLIP_HEADER_LIST.Cancel();
                    }
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Delete)
                {
                    if (IDA_BL_SLIP_HEADER_LIST.IsFocused)
                    {
                        IDA_BL_SLIP_HEADER_LIST.Delete();
                    }
                    else if (idaSLIP_HEADER.IsFocused)
                    {
                        for (int r = 0; r < igrSLIP_LINE.RowCount; r++)
        ...
        private void FCMF0292_Load(object sender, EventArgs e)
        {            
            SLIP_QUERY_STATUS.EditValue = "NON-QUERY";
            ...
            IDA_BL_SLIP_HEADER_LIST.FillSchema();
            idaSLIP_HEADER.FillSchema();
        }

        private void FCMF0292_Shown(object sender, EventArgs e)
        {
            itbSLIP.SelectedIndex = 1;
            itbSLIP.SelectedTab.Focus();
        }
        ...
        private void ibtSUB_FORM_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            {//받을어음
                ...
                FCMF0292_BILL vFCMF0292_BILL = new FCMF0292_BILL(isAppInterfaceAdv1.AppInterface, mDEPT_ID, mDEPT_NAME
                ...
                mEAPF1102.SetProperties(EAPF1102.INIT_TYPE.None, vFCMF0292_BILL, isAppInterfaceAdv1.AppInterface);
                dlgResult = vFCMF0292_BILL.ShowDialog();
                if (dlgResult == DialogResult.OK)
                {
                    //어음금액
                    GL_AMOUNT.EditValue = vFCMF0292_BILL.Get_BILL_AMOUNT;
                    //거래처.
                    Set_Management_Value("CUSTOMER", vFCMF0292_BILL.Get_VENDOR_CODE, vFCMF0292_BILL.Get_VENDOR_NAME);
                    //은행
                    Set_Management_Value("BANK", vFCMF0292_BILL.Get_BANK_CODE, vFCMF0292_BILL.Get_BANK_NAME);
                    //세금계산서발행일
                    Set_Management_Value("VAT_ISSUE_DATE", vFCMF0292_BILL.Get_VAT_ISSUE_DATE, null);
                    //발행일자
                    Set_Management_Value("ISSUE_DATE", vFCMF0292_BILL.Get_ISSUE_DATE, null);
                    //만기일자
                    Set_Management_Value("DUE_DATE", vFCMF0292_BILL.Get_DUE_DATE, null);
                    //만기일자
                    Set_Management_Value("TR_EXPIRATION_DATE", vFCMF0292_BILL.Get_DUE_DATE, null);  
                    //어음번호.
                    Set_Management_Value("RECEIVABLE_BILL", vFCMF0292_BILL.Get_BILL_NUM, String.Format("{0:###,###,###,###,###,###}", vFCMF0292_BILL.Get_BILL_AMOUNT));
                     
                    Init_DR_CR_Amount();    // 차대금액 생성 //
                    Init_Total_GL_Amount(); // 총합계 및 분개 차액 생성 //
                }
                vFCMF0292_BILL.Dispose();
            }
            ...
            {//지급어음
                ...
                if (dlgResult == DialogResult.OK)
                {
                    ...
                    Set_Management_Value("TR_EXPIRATION_DATE", vFCMF0292_BILL.Get_DUE_DATE, null);  
                    //어음번호.
                    Set_Management_Value("PAYABLE_BILL", vFCMF0292_BILL.Get_BILL_NUM, String.Format("{0:###,###,###,###,###,###}", vFCMF0292_BILL.Get_BILL_AMOUNT));
            ...
            else if (iString.ISNull(ACCOUNT_CLASS_TYPE.EditValue) == "AP_VAT".ToString())
            {
                ...
                object mSUPPLY_AMOUNT = Get_Management_Value("SUPPLY_AMOUNT");   //공급가액 설정.
                object mVAT_AMOUNT = Get_Management_Value("VAT_AMOUNT");      //세액 설정.
                object mCOUNT = 0;                          // 매수.
                 
                FCMF0292_ASSET vFCMF0292_ASSET = new FCMF0292_ASSET(isAppInterfaceAdv1.AppInterface, mSLIP_LINE_ID, mSUPPLY_AMOUNT, mVAT_AMOUNT);
                Application.UseWaitCursor = true;
                mEAPF1102.SetProperties(EAPF1102.INIT_TYPE.None, vFCMF0292_ASSET, isAppInterfaceAdv1.AppInterface);
                dlgResult = vFCMF0292_ASSET.ShowDialog();
            ...
            {//외화획득용 구매(공급) 확인서
                ...
                FCMF0292_ITEM_DEAL vFCMF0292_ITEM_DEAL = new FCMF0292_ITEM_DEAL(isAppInterfaceAdv1.AppInterface, mISSUE_NUM, mCURRENCY_CODE
                                                                                , mVENDOR_CODE, mBANK_CODE, mISSUE_DATE);

                mEAPF1102.SetProperties(EAPF1102.INIT_TYPE.None, vFCMF0292_ITEM_DEAL, isAppInterfaceAdv1.AppInterface);
                dlgResult = vFCMF0292_ITEM_DEAL.ShowDialog();
                if (dlgResult == DialogResult.OK)
                {
                    //거래처.
                    MANAGEMENT1.EditValue = vFCMF0292_ITEM_DEAL.Get_VENDOR_CODE;
                    MANAGEMENT1_DESC.EditValue = vFCMF0292_ITEM_DEAL.Get_VENDOR_NAME;
                    //구매(공급)확인번호
                    MANAGEMENT2.EditValue = vFCMF0292_ITEM_DEAL.Get_ISSUE_NUM;
                    MANAGEMENT2_DESC.EditValue = DBNull.Value;

                    REFER4.EditValue = vFCMF0292_ITEM_DEAL.Get_BANK_CODE;
                    REFER4_DESC.EditValue = vFCMF0292_ITEM_DEAL.Get_BANK_NAME;

                    REFER5.EditValue = vFCMF0292_ITEM_DEAL.Get_ISSUE_DATE;                    
                }
                vFCMF0292_ITEM_DEAL.Dispose();
            }
            Application.UseWaitCursor = false;
            this.Cursor = System.Windows.Forms.Cursors.Default;
            Application.DoEvents();
        ...
        {
            ILD_SLIP_TYPE_BL_DOCU.SetLookupParamValue("P_ENABLED_FLAG", "Y"); 
        }
