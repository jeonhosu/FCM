...
namespace FCMF0826
{
    public class XLPrinting
    {
        ...
        private int mCopy_EndRow = 31;
        private int m1stLastRow = 30;  //첫장 최종 인쇄 라인.
        private int m2ndLastRow = 30;  //첫장외 최종 인쇄 라인.

        private int mCurrentRow = 11;       //현재 인쇄되는 row 위치.
        private int mDefaultPageRow = 10;    // 페이지 증가후 PageCount 기본값.
        ...
        {// 그리드의 컬럼에 대한 컬럼인덱스 값 산출
            ...
            pXLColumn = new int[7];
            // 그리드 or 아답터 위치.
            pGDColumn[0] = pGrid.GetColumnToIndex("SUPPLY_YEAR");
            pGDColumn[1] = pGrid.GetColumnToIndex("SUPPLY_MONTH");
            pGDColumn[2] = pGrid.GetColumnToIndex("SUPPLY_DAY");
            pGDColumn[3] = pGrid.GetColumnToIndex("SUPPLY_AMOUNT");
            pGDColumn[4] = pGrid.GetColumnToIndex("VENDOR_NAME");
            pGDColumn[5] = pGrid.GetColumnToIndex("VAT_NUMBER");
            pGDColumn[6] = pGrid.GetColumnToIndex("DOC_NO"); 
        ...
        public void HeaderWrite(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, string pPRINT_TITLE, string pPRINT_SUB_TITLE)
        {// 헤더 인쇄.
            int vXLine = 0;
            int vXLColumn = 0;
