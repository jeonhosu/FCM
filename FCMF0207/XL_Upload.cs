...
namespace FCMF0207
{
    public class XL_Upload
    {
        ...
        public bool LoadXL(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow, InfoSummit.Win.ControlAdv.ISProgressBar pPB, InfoSummit.Win.ControlAdv.ISPrompt pPM)
        {
            ...
            object vObject = null;
            object vSLIP_DATE = string.Empty;
            object vSLIP_NUM = string.Empty;
            object vSLIP_LINE_SEQ = string.Empty;
            object vDEPT_CODE = string.Empty;
            object vPERSON_NUM = string.Empty;
            object vSLIP_TYPE = string.Empty;
            object vACCOUNT_CODE = string.Empty;
            object vACCOUNT_DESC = string.Empty;
            object vACCOUNT_DR_CR = string.Empty;
            object vCURRENCY_CODE = string.Empty; 
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    vSLIP_DATE = ConvertDate(mExcel_Upload.XLGetCell(vRow, 1)); 
                    vSLIP_NUM = mExcel_Upload.XLGetCell(vRow, 2);
                    vSLIP_LINE_SEQ = mExcel_Upload.XLGetCell(vRow, 3);
                    vDEPT_CODE = mExcel_Upload.XLGetCell(vRow, 4);
                    vPERSON_NUM = mExcel_Upload.XLGetCell(vRow, 5);
                    vSLIP_TYPE = mExcel_Upload.XLGetCell(vRow, 7);
                    vACCOUNT_CODE = mExcel_Upload.XLGetCell(vRow, 11);
                    vACCOUNT_DESC = mExcel_Upload.XLGetCell(vRow, 12);
                    vACCOUNT_DR_CR = mExcel_Upload.XLGetCell(vRow, 13);
                    vCURRENCY_CODE = mExcel_Upload.XLGetCell(vRow, 15);
                     

                    if (iString.ISNull(vSLIP_DATE) != string.Empty && iString.ISNull(vSLIP_NUM) != string.Empty)  //사원번호가 있을 경우만 처리.
                    {   
                        try
                        { 
                            pCMD.SetCommandParamValue("P_SLIP_DATE", vSLIP_DATE);
                            pCMD.SetCommandParamValue("P_SLIP_NUM", vSLIP_NUM);
                            pCMD.SetCommandParamValue("P_SLIP_LINE_SEQ", vSLIP_LINE_SEQ); 
                            pCMD.SetCommandParamValue("P_DEPT_CODE", vDEPT_CODE);
                            pCMD.SetCommandParamValue("P_PERSON_NUM", vPERSON_NUM);
                            pCMD.SetCommandParamValue("P_BUDGET_DEPT_CODE", mExcel_Upload.XLGetCell(vRow, 6));
                            pCMD.SetCommandParamValue("P_SLIP_TYPE", vSLIP_TYPE);
                            pCMD.SetCommandParamValue("P_GL_DATE", ConvertDate(mExcel_Upload.XLGetCell(vRow, 8)));
                            pCMD.SetCommandParamValue("P_GL_NUM", mExcel_Upload.XLGetCell(vRow, 9));
                            pCMD.SetCommandParamValue("P_HEADER_REMARK", mExcel_Upload.XLGetCell(vRow, 10));
                            pCMD.SetCommandParamValue("P_ACCOUNT_CODE", vACCOUNT_CODE);
                            pCMD.SetCommandParamValue("P_ACCOUNT_DESC", vACCOUNT_DESC);
                            pCMD.SetCommandParamValue("P_ACCOUNT_DR_CR", vACCOUNT_DR_CR);
                            pCMD.SetCommandParamValue("P_GL_AMOUNT", mExcel_Upload.XLGetCell(vRow, 14));
                            pCMD.SetCommandParamValue("P_CURRENCY_CODE", vCURRENCY_CODE);
                            pCMD.SetCommandParamValue("P_EXCHANGE_RATE", mExcel_Upload.XLGetCell(vRow, 16));
                            pCMD.SetCommandParamValue("P_GL_CURRENCY_AMOUNT", mExcel_Upload.XLGetCell(vRow, 17));

                            pCMD.SetCommandParamValue("P_MANAGEMENT1", mExcel_Upload.XLGetCell(vRow, 18));
                            pCMD.SetCommandParamValue("P_MANAGEMENT2", mExcel_Upload.XLGetCell(vRow, 19));
                            pCMD.SetCommandParamValue("P_REFER1", mExcel_Upload.XLGetCell(vRow, 20));
                            pCMD.SetCommandParamValue("P_REFER2", mExcel_Upload.XLGetCell(vRow, 21));
                            pCMD.SetCommandParamValue("P_REFER3", mExcel_Upload.XLGetCell(vRow, 22));
                            pCMD.SetCommandParamValue("P_REFER4", mExcel_Upload.XLGetCell(vRow, 23));
                            pCMD.SetCommandParamValue("P_REFER5", mExcel_Upload.XLGetCell(vRow, 24));
                            pCMD.SetCommandParamValue("P_REFER6", mExcel_Upload.XLGetCell(vRow, 25));
                            pCMD.SetCommandParamValue("P_REFER7", mExcel_Upload.XLGetCell(vRow, 26));
                            pCMD.SetCommandParamValue("P_REFER8", mExcel_Upload.XLGetCell(vRow, 27));
                            pCMD.SetCommandParamValue("P_REMARK", mExcel_Upload.XLGetCell(vRow, 28));
                            pCMD.SetCommandParamValue("P_BUDGET_EXCEPT_FLAG", mExcel_Upload.XLGetCell(vRow, 29));
                            pCMD.ExecuteNonQuery();
                            if (iString.ISNull(pCMD.GetCommandParamValue("O_STATUS")) == "F")
                            {
                                vMessage = iString.ISNull(pCMD.GetCommandParamValue("O_MESSAGE"));
                                pPM.PromptText = string.Format("Importing :: {0} / {1} *** Acc. Code : {2}({3}) - Line Seq : {4} ** Error : {5}", vRow, (mTotalROW - pStartRow), vACCOUNT_CODE, vACCOUNT_DESC, vSLIP_LINE_SEQ, vMessage);
                                vERR_CNT++;
                                return false;
                            }
                        }
                    ...
                    pPM.PromptText = string.Format("Importing :: {0} / {1} *** Acc. Code :  : {2}({3}) - Line Seq : {4}", vRow, (mTotalROW - 1), vACCOUNT_CODE, vACCOUNT_DESC, vSLIP_LINE_SEQ);
