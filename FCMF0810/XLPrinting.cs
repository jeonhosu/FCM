...
namespace FCMF0810
{
    public class XLPrinting
    {
        ...
        private int mCurrentRowAdd = 6;     //페이지 증가후 기본적으로 증가해야 할 row수.
        ...
        {// 그리드의 컬럼에 대한 컬럼인덱스 값 산출
            ...
            pXLColumn = new int[3];
            // 그리드 or 아답터 위치.
            pGDColumn[0] = pGrid.GetColumnToIndex("ASSET_COUNT");
        ...
        {// 그리드의 컬럼에 대한 컬럼인덱스 값 산출
            ...
            // 그리드 or 아답터 위치.
            pGDColumn[0] = 0;
            pGDColumn[1] = pGrid.GetColumnToIndex("ACQUIRE_DATE");
            pGDColumn[2] = pGrid.GetColumnToIndex("CUSTOMER_DESC");
            pGDColumn[3] = pGrid.GetColumnToIndex("TAX_REG_NO");
            pGDColumn[4] = pGrid.GetColumnToIndex("VAT_ASSET_GB_DESC");
            pGDColumn[5] = pGrid.GetColumnToIndex("GL_AMOUNT");
        ...
        private void IsNewPage(int pPageRowCount)
        {
            ...
            { // pPrintingLine : 현재 출력된 행.
                mIsNewPage = true;
                mCopyLineSUM = CopyAndPaste(mPrinting, mSourceSheet2, mCurrentRow + 1);
            }
            else
            {
                mIsNewPage = false;
