...
namespace SOMF0992
{
    public partial class SOMF0992 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public SOMF0992(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        {
            IGR_TAX_BILL_ISSUE_Y.LastConfirmChanges();
            IDA_TAX_BILL_ISSUE.OraSelectData.AcceptChanges();
            IDA_TAX_BILL_ISSUE.Refillable = true;

            IDA_TAX_BILL_ISSUE.Fill();
            IGR_TAX_BILL_ISSUE_Y.Focus();
        }

        private void Init_Insert()
        {
            IGR_TAX_BILL_ISSUE_Y.SetCellValue("ENABLED_FLAG", "Y");
            IGR_TAX_BILL_ISSUE_Y.SetCellValue("EFFECTIVE_DATE_FR", iDate.ISMonth_1st(DateTime.Today));
        ...
        {
            SOMF0992_DETAIL vSOMF0992_DETAIL = new SOMF0992_DETAIL(this.MdiParent, isAppInterfaceAdv1.AppInterface, "UPDATE", pTAX_BILL_ISSUE_NO);
            vSOMF0992_DETAIL.ShowDialog();
            vSOMF0992_DETAIL.Dispose();
            return true;
        }

        private bool Show_Detail(object pTAX_BILL_ISSUE_NO, object pTAX_BILL_NO, object pHOMETAX_ISSUE_NO)
        {
            ...
            SOMF0992_DETAIL vSOMF0992_DETAIL = new SOMF0992_DETAIL(this.MdiParent, isAppInterfaceAdv1.AppInterface, "FIX", 
        ...
        {
            SOMF0992_DETAIL vSOMF0992_DETAIL = new SOMF0992_DETAIL(this.MdiParent, isAppInterfaceAdv1.AppInterface, "INSERT", string.Empty, string.Empty, string.Empty);
            vSOMF0992_DETAIL.ShowDialog();
        ...
        private void BTN_EXEC_BILL365_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            int vIDX_CHECK_YN = IGR_TAX_BILL_ISSUE_Y.GetColumnToIndex("CHECK_YN");
            int vIDX_ISSUE_DATE = IGR_TAX_BILL_ISSUE_Y.GetColumnToIndex("ISSUE_DATE");
            int vIDX_TAX_BILL_ISSUE_NO = IGR_TAX_BILL_ISSUE_Y.GetColumnToIndex("TAX_BILL_ISSUE_NO");
            int vIDX_DELAY_ISSUE_YN = IGR_TAX_BILL_ISSUE_Y.GetColumnToIndex("DELAY_ISSUE_YN");

            for (int r = 0; r < IGR_TAX_BILL_ISSUE_Y.RowCount; r++)
            {
                if (iConv.ISNull(IGR_TAX_BILL_ISSUE_Y.GetCellValue(r, vIDX_CHECK_YN)) == "Y")
                {
                    vDELAY_ISSUE_YN = "N";
                    vISSUE_DATE = iDate.ISGetDate(IGR_TAX_BILL_ISSUE_Y.GetCellValue(r, vIDX_ISSUE_DATE)).Date;
                    ...
                    IDC_SET_TRANSFER_BILL365.SetCommandParamValue("W_TAX_BILL_ISSUE_NO", IGR_TAX_BILL_ISSUE_Y.GetCellValue(r, vIDX_TAX_BILL_ISSUE_NO));
        ...
        private void BTN_CANCEL_BILL365_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            for (int r = 0; r < IGR_TAX_BILL_ISSUE_Y.RowCount; r++)
            {
                if (iConv.ISNull(IGR_TAX_BILL_ISSUE_Y.GetCellValue(r, vIDX_CHECK_YN)) == "Y")
                {
                    ...
                    IDC_CANCEL_TRANSFER_BILL365.SetCommandParamValue("W_TAX_BILL_ISSUE_NO", IGR_TAX_BILL_ISSUE_Y.GetCellValue(r, vIDX_TAX_BILL_ISSUE_NO));
        ...
        {
            Show_Detail(IGR_TAX_BILL_ISSUE_Y.GetCellValue("TAX_BILL_ISSUE_NO"), 
                        IGR_TAX_BILL_ISSUE_Y.GetCellValue("TAX_BILL_NO"), 
                        IGR_TAX_BILL_ISSUE_Y.GetCellValue("HOMETAX_ISSUE_NO"));
        }

        private void IGR_TAX_BILL_ISSUE_CellDoubleClick(object pSender)
        {
            if (IGR_TAX_BILL_ISSUE_Y.RowIndex < 0)
            {
                return;
            }
        ...
        private void W_START_DATE_CurrentEditValidated(object pSender, ISEditAdvValidatedEventArgs e)
        {
        ...
        {
            decimal vVAT_RATE = iConv.ISDecimaltoZero(IGR_TAX_BILL_ISSUE_Y.GetCellValue("VAT_RATE"));
            decimal vQTY = 0;
            decimal vUNIT_PRICE = 0;
            decimal vSUPPLY_AMOUNT = 0;
            decimal vVAT_AMOUNT = 0;
            if (e.ColIndex == IGR_TAX_BILL_ISSUE_LINE_Y.GetColumnToIndex("QTY"))
            {
                vQTY = iConv.ISDecimaltoZero(e.NewValue,1);
                vUNIT_PRICE = iConv.ISDecimaltoZero(IGR_TAX_BILL_ISSUE_LINE_Y.GetCellValue("UNIT_PRICE"), 0);
                vSUPPLY_AMOUNT = vQTY * vUNIT_PRICE;
                vVAT_AMOUNT = Math.Round(vSUPPLY_AMOUNT * vVAT_RATE);

                IGR_TAX_BILL_ISSUE_LINE_Y.SetCellValue("SUPPLY_AMOUNT", vSUPPLY_AMOUNT);
                IGR_TAX_BILL_ISSUE_LINE_Y.SetCellValue("VAT_AMOUNT", vSUPPLY_AMOUNT);
            }
            else if (e.ColIndex == IGR_TAX_BILL_ISSUE_LINE_Y.GetColumnToIndex("UNIT_PRICE"))
            {
                vQTY = iConv.ISDecimaltoZero(IGR_TAX_BILL_ISSUE_LINE_Y.GetCellValue("QTY"), 1);
            ...
            else if (e.ColIndex == IGR_TAX_BILL_ISSUE_LINE_Y.GetColumnToIndex("SUPPLY_AMOUNT"))
            {
        ...
        {
            int vIDX_CHECK_YN = IGR_TAX_BILL_ISSUE_Y.GetColumnToIndex("CHECK_YN");
            for (int vRow = 0; vRow < IGR_TAX_BILL_ISSUE_Y.RowCount; vRow++)
            {
                IGR_TAX_BILL_ISSUE_Y.SetCellValue(vRow, vIDX_CHECK_YN, V1_CHECK_FLAG.CheckBoxString);  
            }
        ...
        {
            decimal vVAT_AMOUNT = Sync_VAT_Amount(IGR_TAX_BILL_ISSUE_Y.GetCellValue("VAT_TAX_TYPE"), IGR_TAX_BILL_ISSUE_Y.GetCellValue("SUPPLY_AMOUNT"));
            IGR_TAX_BILL_ISSUE_Y.SetCellValue("VAT_AMOUNT", vVAT_AMOUNT);
        }
