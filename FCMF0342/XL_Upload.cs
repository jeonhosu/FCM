        ...
        public bool LoadXL_10(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, int pStartRow)
        {
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    vType = pAdapter.CurrentRow.Table.Columns["ASSET_DESC"].DataType;
                    if (vType.Name == "String")
                    {
                        vASSET_DESC = mExcel_Upload.XLGetCell(vRow, 3);  //KEY 값.
                    }
                    else
                    {
                    ...
                    if (iString.ISNull(vASSET_DESC).Trim() != string.Empty)  //자산명 있을 경우만 처리.
                    {
