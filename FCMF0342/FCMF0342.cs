...
namespace FCMF0342
{
    public partial class FCMF0342 : Office2007Form
    {       
        #region ----- Variables -----
        ...
        public FCMF0342(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        {            
            ...
            IDA_ASSET_MASTER_PMS.OraSelectData.AcceptChanges();
            IDA_ASSET_MASTER_PMS.Refillable = true;

            igrASSET_HISTORY.LastConfirmChanges();
            IDA_ASSET_HISTORY_PMS.OraSelectData.AcceptChanges();
            IDA_ASSET_HISTORY_PMS.Refillable = true;

            IGR_ASSET_DPR_ACCOUNT.LastConfirmChanges();
            IDA_ASSET_DPR_ACCOUNT_PMS.OraSelectData.AcceptChanges();
            IDA_ASSET_DPR_ACCOUNT_PMS.Refillable = true;

            IGR_ASSET_DPR_RATE.LastConfirmChanges();
            IDA_ASSET_DPR_RATE.OraSelectData.AcceptChanges();
            IDA_ASSET_DPR_RATE.Refillable = true;
 
            IDA_ASSET_MASTER_PMS.Fill();
        ...
        private void Insert_Asset_History()
        {
            ...
            igrASSET_HISTORY.SetCellValue("PROJECT_CODE", PROJECT_CODE.EditValue);
            igrASSET_HISTORY.SetCellValue("PROJECT_NAME", PROJECT_NAME.EditValue);
            igrASSET_HISTORY.SetCellValue("PROJECT_LOCATION", PROJECT_LOCATION.EditValue);
            igrASSET_HISTORY.SetCellValue("PROJECT_LOCATION_DESC", PROJECT_LOCATION_DESC.EditValue);
            igrASSET_HISTORY.SetCellValue("PROJECT_DEPT", PROJECT_DEPT.EditValue);
            igrASSET_HISTORY.SetCellValue("PROJECT_DEPT_NAME", PROJECT_DEPT_NAME.EditValue); 
        ...
        {            
            IGR_ASSET_DPR_ACCOUNT.SetCellValue("PROJECT_CODE", PROJECT_CODE.EditValue);
            IGR_ASSET_DPR_ACCOUNT.SetCellValue("PROJECT_NAME", PROJECT_NAME.EditValue);
            IGR_ASSET_DPR_ACCOUNT.SetCellValue("PROJECT_LOCATION", PROJECT_LOCATION.EditValue);
            IGR_ASSET_DPR_ACCOUNT.SetCellValue("PROJECT_LOCATION_DESC", PROJECT_LOCATION_DESC.EditValue);
            IGR_ASSET_DPR_ACCOUNT.SetCellValue("PROJECT_DEPT", PROJECT_DEPT.EditValue);
            IGR_ASSET_DPR_ACCOUNT.SetCellValue("PROJECT_DEPT_NAME", PROJECT_DEPT_NAME.EditValue); 
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    if (IDA_ASSET_MASTER_PMS.IsFocused)
                    {
                        IDA_ASSET_MASTER_PMS.AddOver();
                        Insert_Asset_Master();
                    }
                    else if (IDA_ASSET_HISTORY_PMS.IsFocused)
                    {
                        IDA_ASSET_HISTORY_PMS.AddOver();
                        Insert_Asset_History();
                    }
                    else if (IDA_ASSET_DPR_ACCOUNT_PMS.IsFocused)
                    {
                        IDA_ASSET_DPR_ACCOUNT_PMS.AddOver();
                        Insert_DPR_ACCOUNT();
                    }
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.AddUnder)
                {
                    if (IDA_ASSET_MASTER_PMS.IsFocused)
                    {
                        IDA_ASSET_MASTER_PMS.AddUnder();
                        Insert_Asset_Master();
                    }
                    else if (IDA_ASSET_HISTORY_PMS.IsFocused)
                    {
                        IDA_ASSET_HISTORY_PMS.AddUnder();
                        Insert_Asset_History();
                    }
                    else if (IDA_ASSET_DPR_ACCOUNT_PMS.IsFocused)
                    {
                        IDA_ASSET_DPR_ACCOUNT_PMS.AddUnder();
                        Insert_DPR_ACCOUNT();
                    }
                    else if (IDA_ASSET_DPR_RATE.IsFocused)
                    {
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Cancel)
                {
                    if (IDA_ASSET_MASTER_PMS.IsFocused)
                    {
                        ...
                        IDA_ASSET_DPR_ACCOUNT_PMS.Cancel(); 
                        IDA_ASSET_HISTORY_PMS.Cancel();
                        IDA_ASSET_MASTER_PMS.Cancel();
                    }
                    else if (IDA_ASSET_HISTORY_PMS.IsFocused)
                    {
                        IDA_ASSET_HISTORY_PMS.Cancel();
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Delete)
                {
                    if (IDA_ASSET_MASTER_PMS.IsFocused)
                    {
                        IDA_ASSET_MASTER_PMS.Delete();
                    }
                    else if (IDA_ASSET_HISTORY_PMS.IsFocused)
                    {
                        IDA_ASSET_HISTORY_PMS.Delete();
                    }
                    else if (IDA_ASSET_DPR_ACCOUNT_PMS.IsFocused)
                    {
                        IDA_ASSET_DPR_ACCOUNT_PMS.Delete();
                    }
                    else if (IDA_ASSET_DPR_RATE.IsFocused)
                    {
        ...
        private void FCMF0342_Load(object sender, EventArgs e)
        {
            W_CIP_ALL.CheckedState = ISUtil.Enum.CheckedState.Checked;
            W_CIP_FLAG.EditValue = W_CIP_ALL.RadioButtonString; 
            
            IDA_ASSET_MASTER_PMS.FillSchema();
            IDA_ASSET_HISTORY_PMS.FillSchema();
            IDA_ASSET_DPR_ACCOUNT_PMS.FillSchema();
            IDA_ASSET_DPR_RATE.FillSchema();
        }

        private void FCMF0342_Shown(object sender, EventArgs e)
        {
        ...
        {
            if (IDA_ASSET_MASTER_PMS.CurrentRow.RowState == DataRowState.Added)
            {
        ...
        {
            IDA_ASSET_DPR_ACCOUNT_PMS.Fill();
        }
        ...
        private void ILA_PROJECT_LOCATION_PrePopupShow(object pSender, ISLookupPopupShowEventArgs e)
        {
            ILD_PROJECT_LOCATION.SetLookupParamValue("P_PROJECT_CODE", PROJECT_CODE.EditValue);
        }

        private void ILA_PROJECT_LOCATION_H_PrePopupShow(object pSender, ISLookupPopupShowEventArgs e)
        {
            ILD_PROJECT_LOCATION.SetLookupParamValue("P_PROJECT_CODE", igrASSET_HISTORY.GetCellValue("PROJECT_CODE"));
        }

        private void ILA_PROJECT_LOCATION_ACC_PrePopupShow(object pSender, ISLookupPopupShowEventArgs e)
        {
            ILD_PROJECT_LOCATION.SetLookupParamValue("P_PROJECT_CODE", IGR_ASSET_DPR_ACCOUNT.GetCellValue("PROJECT_CODE"));
        }
