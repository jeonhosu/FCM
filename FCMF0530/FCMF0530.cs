...
namespace FCMF0530
{
    public partial class FCMF0530 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public FCMF0530(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void SearchDB()
        {
            ...
            int vIDX_Col = IGR_BALANCE_APAR_SUM.GetColumnToIndex("VENDOR_CODE");
            string vVENDOR_CODE = iString.ISNull(IGR_BALANCE_APAR_SUM.GetCellValue("VENDOR_CODE"));
            
            IDA_BALANCE_APAR_SUM.Fill();
            ...
            for (int nRow = 0; nRow < IGR_BALANCE_APAR_SUM.RowCount; nRow++)
            {
                if (vVENDOR_CODE == iString.ISNull(IGR_BALANCE_APAR_SUM.GetCellValue(nRow, vIDX_Col)))
                {
                    IGR_BALANCE_APAR_SUM.CurrentCellMoveTo(nRow, vIDX_Col);
                    IGR_BALANCE_APAR_SUM.CurrentCellActivate(nRow, vIDX_Col); 
                    return;
                }
            }
            IGR_BALANCE_APAR_SUM.Focus(); 
        }
        ...
        {
            int mSLIP_HEADER_ID = iString.ISNumtoZero(IGR_BALANCE_APAR_DETAIL.GetCellValue("SLIP_HEADER_ID"));
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    if (IDA_BALANCE_APAR_SUM.IsFocused)
                    {
                        ExcelExport(IGR_BALANCE_APAR_SUM, "apar_sum");
                    }
                    else if (IDA_BALANCE_APAR_DETAIL.IsFocused)
                    {
                        ExcelExport(IGR_BALANCE_APAR_DETAIL, "apar_detail");
                    }
                }
            }
        }
        ...
        private void FCMF0530_Load(object sender, EventArgs e)
        {
            // 전표저장시 자동 승인 여부
            ...
            V_RB_APAR_ALL.CheckedState = ISUtil.Enum.CheckedState.Checked;
            V_BATCH_TYPE.EditValue = V_RB_APAR_ALL.RadioCheckedString;
        }

        private void FCMF0530_Shown(object sender, EventArgs e)
        {
            W_BALANCE_DATE.EditValue = DateTime.Today; 
        }

        private void V_RB_APAR_ALL_CheckChanged(object sender, EventArgs e)
        {
            if (V_RB_APAR_ALL.CheckedState == ISUtil.Enum.CheckedState.Checked)
            {
                V_BATCH_TYPE.EditValue = V_RB_APAR_ALL.RadioCheckedString;
            }
        }
        ...
        {
            ILD_ACCOUNT_CONTROL_BATCH.SetLookupParamValue("W_ACCOUNT_CODE", W_ACCOUNT_CODE.EditValue);
            ILD_ACCOUNT_CONTROL_BATCH.SetLookupParamValue("W_ENABLED_YN", "Y");
        }
        ...
        {
            ILD_MANAGEMENT_TYPE.SetLookupParamValue("W_ACCOUNT_CONTROL_ID", IGR_BALANCE_APAR_SUM.GetCellValue("ACCOUNT_CONTROL_ID"));
