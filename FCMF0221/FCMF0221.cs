...
namespace FCMF0221
{
    public partial class FCMF0221 : Office2007Form
    {
        ...
        private void Insert_Loan_Plan()
        {
            ...
            object vCURRENCY_CODE = L_CURRENCY_CODE.EditValue;
            if(iConv.ISNull(vCURRENCY_CODE) == string.Empty)
            {
                vCURRENCY_CODE = CURRENCY_CODE.EditValue;
            }
            IGR_LOAN_PLAN.SetCellValue("CURRENCY_CODE", vCURRENCY_CODE);
            IGR_LOAN_PLAN.Focus();
        }
        ...
        {
            if (iConv.ISNull(L_CURRENCY_CODE.EditValue) == string.Empty || L_CURRENCY_CODE.EditValue.ToString() == mCurrency_Code)
            {
                L_EXCHANGE_RATE.EditValue = DBNull.Value;

                if (iConv.ISDecimaltoZero(LIMIT_CURR_AMOUNT.EditValue) != Convert.ToDecimal(0))
                {
        ...
        {
            if (iConv.ISNull(L_CURRENCY_CODE.EditValue) != mCurrency_Code)
            {
                LIMIT_AMOUNT.EditValue = iConv.ISDecimaltoZero(LIMIT_CURR_AMOUNT.EditValue) * iConv.ISDecimaltoZero(L_EXCHANGE_RATE.EditValue);
            }
        }
        ...
        {
            W_STD_DATE.EditValue = iDate.ISGetDate(DateTime.Today);
        ...
        private void ilaLOAN_KIND_SelectedRowData(object pSender)
        {
            ...
            {
                if (iConv.ISNull(L_ISSUE_DATE.EditValue) == string.Empty)
                {
                    L_ISSUE_DATE.EditValue = DateTime.Today;
                }
                if (iConv.ISNull(L_DUE_DATE.EditValue) == string.Empty)
                {
                    L_DUE_DATE.EditValue = DateTime.Today;
                }
                if (iConv.ISNull(L_CURRENCY_CODE.EditValue) == string.Empty)
                {
        ...
        private void idaLONE_MASTER_PreRowUpdate(ISPreRowUpdateEventArgs e)
        {
            ...
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_90004", String.Format("&&FIELD_NAME:={0}", Get_Edit_Prompt(CHANGE_DATE))), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
                CHANGE_DATE.Focus();
                return;
            }
