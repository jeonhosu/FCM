...
namespace FCMF0759
{
    public class XLPrinting
    {
        ...
        private void SetArray1(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn)
        {
            ...
            // 그리드 or 아답터 위치.
            pGDColumn[0] = pGrid.GetColumnToIndex("ITEM_DESC");
            pGDColumn[1] = pGrid.GetColumnToIndex("MONTH_1");
            pGDColumn[2] = pGrid.GetColumnToIndex("MONTH_2");
            pGDColumn[3] = pGrid.GetColumnToIndex("MONTH_3");
            pGDColumn[4] = pGrid.GetColumnToIndex("MONTH_4");
            pGDColumn[5] = pGrid.GetColumnToIndex("MONTH_5");
            pGDColumn[6] = pGrid.GetColumnToIndex("MONTH_6");
            pGDColumn[7] = pGrid.GetColumnToIndex("MONTH_7");
            pGDColumn[8] = pGrid.GetColumnToIndex("MONTH_8");
            pGDColumn[9] = pGrid.GetColumnToIndex("MONTH_9");
            pGDColumn[10] = pGrid.GetColumnToIndex("MONTH_10");
            pGDColumn[11] = pGrid.GetColumnToIndex("MONTH_11");
            pGDColumn[12] = pGrid.GetColumnToIndex("MONTH_12");
            pGDColumn[13] = pGrid.GetColumnToIndex("YEAR_AMOUNT"); 
        }
        ...
        {// 실제 호출되는 부분.
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    ...
                    {
                        vMessage = string.Format("Printing : {0}/{1}", r, vTotalRow);
                        mAppInterface.OnAppMessageEvent(vMessage);
                        System.Windows.Forms.Application.DoEvents();

                        mCurrentRow = XLLine_1(pGrid, r, mCurrentRow, vGDColumn);  // 현재 위치 인쇄 후 다음 인쇄행 리턴.
                        vPageRowCount = vPageRowCount + 1;
