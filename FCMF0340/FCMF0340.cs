...
namespace FCMF0340
{
    public partial class FCMF0340 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public FCMF0340(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void SearchDB()
        {
            ...
            if (TB_MAIN.SelectedTab.TabIndex == TP_CREATE_SLIP.TabIndex)
            {
                if (IGR_DPR_SLIP_DETAIL.RowCount > 0)
                {
                    ...
                    IDA_DPR_HISTORY_PMS.OraSelectData.AcceptChanges();
                    IDA_DPR_HISTORY_PMS.Refillable = true;
                }

                IDA_DPR_HISTORY_PMS.Fill();
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    if (IDA_DPR_HISTORY_PMS.IsFocused)
                    {
                        IDA_DPR_HISTORY_PMS.Cancel();
                    }
                }
        ...
        private void FCMF0340_Load(object sender, EventArgs e)
        {

        }

        private void FCMF0340_Shown(object sender, EventArgs e)
        {
        ...
        private void BTN_SET_SLIP_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            //선택된 처리대상에 대해 전표 전송//
            IDC_CREATE_DPR_SLIP_PMS.ExecuteNonQuery();
            mSTATUS = IDC_CREATE_DPR_SLIP_PMS.GetCommandParamValue("O_STATUS").ToString();
            if (IDC_CREATE_DPR_SLIP_PMS.ExcuteError || mSTATUS == "F")
            {
                isDataTransaction1.RollBack();
                mMESSAGE = iString.ISNull(IDC_CREATE_DPR_SLIP_PMS.GetCommandParamValue("O_MESSAGE"));
                if (mMESSAGE != string.Empty)
                {
                    Application.UseWaitCursor = false;
                    this.Cursor = Cursors.Default;
