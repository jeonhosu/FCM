...
namespace FCMF0629
{
    public class XLPrinting
    {
        ...
        private decimal mSUM_APPLY_AMOUNT = 0;  //예산 합계
        private decimal mAPPLY_AMOUNT = 0;      //차액 합계  
        private decimal mREMAIN_AMOUNT = 0;     //잔액 합계
        ...
        {// 헤더 인쇄.
            ...
            try
            {
                ...
                vXLine = 10;
                vXLColumn = 4;
                vObject = pHeader.CurrentRow["BUDGET_APPLY_NUM"];
                if (iString.ISNull(vObject) != string.Empty)
                {
                    vPrintString = string.Format("{0}", vObject);
                }
        ...
        {// pGridRow : 그리드의 현재 읽는 행, pXLine : 엑셀의 인쇄해야 하는 행
            ...
            try
            {
                ...
                //[APPLY_AMOUNT]
                vXLColumn = 10;
                vObject = pRow["APPLY_AMOUNT"];
                if (iString.ISDecimal(vObject) == true)
                {
                    vConvertString = string.Format("{0:###,###,###,###,###,###,###}", vObject);
                    mAPPLY_AMOUNT = mAPPLY_AMOUNT + iString.ISDecimaltoZero(vObject);
                }
                else
                {
                    vConvertString = string.Empty;
                ...
                //[APPLY_SUM_AMOUNT]
                vXLColumn = 13;
                vObject = pRow["APPLY_SUM_AMOUNT"];
                if (iString.ISDecimal(vObject) == true)
                {
                    vConvertString = string.Format("{0:###,###,###,###,###,###,###}", vObject);
                    mSUM_APPLY_AMOUNT = mSUM_APPLY_AMOUNT + iString.ISDecimaltoZero(vObject);
                }
                else
                {
                    vConvertString = string.Empty;
                ...
                {
                    vConvertString = string.Format("{0:###,###,###,###,###,###,###}", vObject);
                    mREMAIN_AMOUNT = mREMAIN_AMOUNT + iString.ISDecimaltoZero(vObject);
                }
                else
                {
                    vConvertString = string.Empty;
        ...
        private void SumWrite(int pPrintingLine)
        {
            ...
            if (mPageNumber == 1)
            {
                ...
                //BACK COLOR.
                mPrinting.XLCellColorBrush(vLINE, 7, vLINE, 16, System.Drawing.Color.Silver);
                ...
                //신청금액합계
                vAmount = string.Format("{0:#,###,###,###,###,###,###,###,###,##0}", mAPPLY_AMOUNT);
                mPrinting.XLSetCell(vLINE, 10, vAmount);

                //누계합계
                vAmount = string.Format("{0:#,###,###,###,###,###,###,###,###.####}", mSUM_APPLY_AMOUNT);
                mPrinting.XLSetCell(vLINE, 13, vAmount);

                //잔액합계
                vAmount = string.Format("{0:#,###,###,###,###,###,###,###,###.####}", mREMAIN_AMOUNT);
                mPrinting.XLSetCell(vLINE, 16, vAmount);
