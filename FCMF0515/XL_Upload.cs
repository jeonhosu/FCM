...
namespace FCMF0515
{
    public class XL_Upload
    {
        ...
        public bool LoadXL(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, int pStartRow)
        {
            ...
            object vBILL_NUM = string.Empty;

            int vADRow = 0;
            int vADCol = 0;
            mTotalCOLUMN = pAdapter.OraSelectData.Columns.Count -1;
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    vType = pAdapter.CurrentRow.Table.Columns[0].DataType;
                    //vType = pAdapter.CurrentRow.Table.Columns["BILL_NUM"].DataType;
                    if (vType.Name == "String")
                    {
                        vObject = mExcel_Upload.XLGetCell(vRow, 1);  //어음번호.
                        vBILL_NUM = iString.ISNull(vObject);
                    }
                    else 
                    {
                        vBILL_NUM = string.Empty;
                        pAdapter.Delete();
                    }
                    if (iString.ISNull(vBILL_NUM) != string.Empty)  //기표일자가 있을 경우만 처리.
                    {                        
