...
namespace FCMF0514
{
    public class XL_Upload
    {
        ...
        public bool LoadXL(InfoSummit.Win.ControlAdv.ISDataCommand pCMD, int pStartRow)
        {
            ...
            string vSTATUS = "F";
            string vMESSAGE = string.Empty;
            string vBANK_ACCOUNT_CODE = string.Empty;
            ...
            try
            {
                for (int vRow = pStartRow; vRow < mTotalROW; vRow++)
                {
                    ...
                    vBANK_ACCOUNT_CODE = iString.ISNull(vObject);

                    if (vBANK_ACCOUNT_CODE != string.Empty)  //사원번호가 있을 경우만 처리.
                    {
                        pCMD.SetCommandParamValue("P_BANK_ACCOUNT_CODE", vBANK_ACCOUNT_CODE);
                        ...
                        vConvertDate = ConvertDate(vObject);
                        pCMD.SetCommandParamValue("P_TRX_DATE", vConvertDate);

                        vObject = mExcel_Upload.XLGetCell(vRow, 4);
                        vConvertString = iString.ISNull(vObject);
                        vConvertString = vConvertString.Trim();
                        pCMD.SetCommandParamValue("P_DEPOSIT_PLAN_TYPE", vConvertString);
