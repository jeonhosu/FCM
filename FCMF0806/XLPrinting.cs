...
namespace FCMF0806
{
    public class XLPrinting
    {
        ...
        private int mPrintingLastRow = 31;  //최종 인쇄 라인.

        private int mCurrentRow = 17;       //현재 인쇄되는 row 위치.
        private int mDefaultPageRow = 9;    //페이지 skip후 적용되는 기본 PageCount 기본값.
        private int mCurrentRowAdd = 10;     //페이지 skip후 기본적으로 증가하는 현재 row 값.
        ...
        {// 그리드의 컬럼에 대한 컬럼인덱스 값 산출
            ...
            // 그리드 or 아답터 위치.
            pGDColumn[0] = 0;
            pGDColumn[1] = pGrid.GetColumnToIndex("CREDITCARD_NUM");
            pGDColumn[2] = pGrid.GetColumnToIndex("TAX_REG_NO");
            pGDColumn[3] = pGrid.GetColumnToIndex("VAT_COUNT");
            pGDColumn[4] = pGrid.GetColumnToIndex("GL_AMOUNT");
            pGDColumn[5] = pGrid.GetColumnToIndex("VAT_AMOUNT");
        ...
        public void HeaderWrite(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, object pPERIOD)
        {// 헤더 인쇄.
            int vXLine = 0;
            int vXLColumn = 0;
        ...
        {// 헤더 인쇄.
            ...
            int vIDX_VAT_TAX_CLASS = pGrid.GetColumnToIndex("VAT_TAX_CLASS");
            ...
            { // 원본을 복사해서 타겟 에 복사해 넣음.(
                ...
                for (int i = 0; i < pGrid.RowCount; i++)
                {
                    ...
                    if ("TOTAL" == iString.ISNull(pGrid.GetCellValue(i, vIDX_VAT_TAX_CLASS)))
                    {//총합계
                        vXLine = 9;
                    }
                    else if ("CARD_ETC" == iString.ISNull(pGrid.GetCellValue(i, vIDX_VAT_TAX_CLASS)))
                    {//신용카드.
                        vXLine = 13;
                    }
                    else if ("CASH" == iString.ISNull(pGrid.GetCellValue(i, vIDX_VAT_TAX_CLASS)))
                    {//현금영수증.
                        vXLine = 10;
                    }
        ...
        {// pGridRow : 그리드의 현재 읽는 행, pXLine : 엑셀의 인쇄해야 하는 행. pGDColumn : 그리드 위치, pXLColumn : 엑셀 위치.
            ...
            { // 원본을 복사해서 타겟 에 복사해 넣음.(
                ...
                // 거래건수 합계;
                mTOT_COUNT = mTOT_COUNT + vConvertDecimal;
                //4 - 공급가액
                ...
                // 공급가액 합계;
                mTOT_GL_AMOUNT = mTOT_GL_AMOUNT + vConvertDecimal;
                //5-세액
                ...
                // 세액 합계;
                mTOT_VAT_AMOUNT = mTOT_VAT_AMOUNT + vConvertDecimal;
                //-------------------------------------------------------------------
                vXLine = vXLine + 1;
                //-------------------------------------------------------------------
            }
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid_Detail, string pPeriod)
        {// 실제 호출되는 부분.
            ...
            try
            {
                if (pGrid.RowCount > 0)
                {
                    ...
                    SetArray0(pGrid, out vGDColumn, out vXLColumn);
                ...
                if (vTotalRow > 0)
                {
                    SetArray1(pGrid_Detail, out vGDColumn, out vXLColumn);
