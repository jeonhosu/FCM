...
namespace FCMF0213
{
    public partial class FCMF0213 : Office2007Form
    {
        ...
        {
            int mIDX_Col = igrSLIP_BUDGET_LIST.GetColumnToIndex("SELECT_YN");
            
            object mCheck_YN = CB_SELECT_YN.CheckBoxValue;
            for (int r = 0; r < igrSLIP_BUDGET_LIST.RowCount; r++)
            {
                igrSLIP_BUDGET_LIST.SetCellValue(r, mIDX_Col, mCheck_YN);
            }
        }
        ...
        private void XLPrinting_Main()
        {
            ...
            else
            {
                ...
                if (vCountRowGrid > 0)
                {
                    ...
                    int vIDX_SELECT_YN = igrSLIP_BUDGET_LIST.GetColumnToIndex("SELECT_YN");
                    int vIDX_HEADER_ID = igrSLIP_BUDGET_LIST.GetColumnToIndex("HEADER_ID");
                    int vIDX_SLIP_DATE = igrSLIP_BUDGET_LIST.GetColumnToIndex("SLIP_DATE");
                    int vIDX_SLIP_NUM = igrSLIP_BUDGET_LIST.GetColumnToIndex("SLIP_NUM");

                    //-------------------------------------------------------------------------------------
                    for (int vRow = 0; vRow < vCountRowGrid; vRow++)
                    {
                        object vSELECT_YN = igrSLIP_BUDGET_LIST.GetCellValue(vRow, vIDX_SELECT_YN);
                        if (iString.ISNull(vSELECT_YN) == "Y")
                        {
                            igrSLIP_BUDGET_LIST.CurrentCellMoveTo(vRow, vIDX_SELECT_YN);
                            igrSLIP_BUDGET_LIST.CurrentCellActivate(vRow, vIDX_SELECT_YN);

                            vSlip_Header_id = igrSLIP_BUDGET_LIST.GetCellValue(vRow, vIDX_HEADER_ID);
                            vSlip_Date = string.Format("{0:yyyy-MM-dd}", igrSLIP_BUDGET_LIST.GetCellValue(vRow, vIDX_SLIP_DATE));
                            vSlip_Num = igrSLIP_BUDGET_LIST.GetCellValue(vRow, vIDX_SLIP_NUM);
                             
                            AssmblyRun_Manual("FCMF0212", vSlip_Header_id, vSlip_Date, vSlip_Num);
                            igrSLIP_BUDGET_LIST.SetCellValue(vRow, vIDX_SELECT_YN, "N");
                        }
                    }
                    //-------------------------------------------------------------------------------------
                } 
        ...
        private void XLPrinting(string pOutput_Type)
        {
                ...
                (itbSLIP.SelectedIndex == 1 && iString.ISNull(HEADER_ID.EditValue) != string.Empty))
            {
                ...
                {
                    //-------------------------------------------------------------------------------------
                    xlPrinting.OpenFileNameExcel = "FCMF0213_001.xlsx";
                    //-------------------------------------------------------------------------------------
        ...
        private void XLPrinting_BSK(string pOutput_Type)
        {
                ...
                (itbSLIP.SelectedIndex == 1 && iString.ISNull(HEADER_ID.EditValue) != string.Empty))
            {
                ...
                {
                    //-------------------------------------------------------------------------------------
                    xlPrinting.OpenFileNameExcel = "FCMF0213_011.xlsx";
                    //-------------------------------------------------------------------------------------
        ...
        private void FCMF0213_Load(object sender, EventArgs e)
        {
            SLIP_QUERY_STATUS.EditValue = "NON-QUERY";
            ...
            CB_DOC_ATT_FLAG.BringToFront();
            BTN_FILE_ATTACH.BringToFront();
            BTN_APPR_STEP.BringToFront();
        ...
        private void FCMF0213_Shown(object sender, EventArgs e)
        {
            Application.DoEvents();
        ...
        {
            //전표 목록에서 파일 보기.
            F_SLIP_DATE.EditValue = igrSLIP_BUDGET_LIST.GetCellValue("SLIP_DATE");
            F_SLIP_NUM.EditValue = igrSLIP_BUDGET_LIST.GetCellValue("SLIP_NUM");
