...
namespace FCMF0242
{
    public class XLPrinting
    {
        ...
        private string mSourceSheet3 = "Source3";
        ...
        private int mPrintingLastRow = 46;  //실제 데이터 인쇄 최종 라인.

        private int mCurrentRow = 6;        //실제 인쇄되는 row 위치.
        private int mDefaultPageRow = 2;    //페이지 skip후 적용되는 기본 PageCount 기본값.
         
        // 인쇄 - 외화 인쇄정보.
        private int mCopy_StartCol2 = 1;
        private int mCopy_StartRow2 = 1;
        private int mCopy_EndCol2 = 48;
        private int mCopy_EndRow2 = 47; 
        ...
        public void HeaderWrite1(object pGL_PERIOD)
        {// 헤더 인쇄.
            int vXLine = 0;
            int vXLColumn = 0;
            ...
            {                
                ...
                vXLine = 4;
                vXLColumn = 6;
                mPrinting.XLSetCell(vXLine, vXLColumn, pGL_PERIOD);
        ...
        private int LineWrite1(System.Data.DataRow pRow, int pXLine)
        ...
        private void LineWrite_Total(System.Data.DataRow pRow, int pXLine)
        {// pGridRow : 그리드의 현재 읽는 행, pXLine : 엑셀의 인쇄해야 하는 행
            int vXLine = pXLine; //엑셀에 내용이 표시되는 행 번호
            int vXLColumn = 0;
            ...
            try
            {
                ...
                mPrinting.XLSetCell(1, vXLColumn, vConvertString); 
            }
            catch (System.Exception ex)
            {
                mMessageError = ex.Message;
        ...
        public int ExcelWrite1(string pGL_PERIOD, InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter)
        {// 실제 호출되는 부분.
            ...
            mDefaultPageRow = 2;    // 페이지 증가후 PageCount 기본값.
            mPrintingLastRow = 46;  //최종 인쇄 라인.
            mCurrentRow = 6;
            int vPageRowCount = mCurrentRow;
            int vCurrRow = 0;
            int vTotalRow = 0;
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    HeaderWrite1(pGL_PERIOD);

                    // 원본을 복사해서 타깃쉬트에 붙여 넣는다.
                    mCopyLineSUM = CopyAndPaste(mPrinting, mSourceSheet1);
                    ...
                    {
                        vCurrRow++;
                        vMessage = string.Format("Printing : {0}/{1}", vCurrRow, vTotalRow);
                        mAppInterface.OnAppMessageEvent(vMessage);
                        System.Windows.Forms.Application.DoEvents();

                        if (iString.ISNull(vRow["SUMMARY_FLAG"]) == "T")
                        {
                            LineWrite_Total(vRow, mCurrentRow);
                            CopyAndPaste_Total(mPrinting, mCurrentRow);
                        }
                        else
                        {
                            mCurrentRow = LineWrite1(vRow, mCurrentRow); // 현재 위치 인쇄 후 다음 인쇄행 리턴.
        ...
        private void IsNewPage(int pPageRowCount)
        {
            ...
            { // pPrintingLine : 현재 출력된 행.
                mIsNewPage = true;
                mCopyLineSUM = CopyAndPaste(mPrinting, mSourceSheet2);
            }
            else
            {
                mIsNewPage = false;
        ...
        private int CopyAndPaste(XL.XLPrint pPrinting, string pSourceSheet)
        {
            int vPasteEndRow = mCopyLineSUM + mCopy_EndRow;
            string vActiveSheet = pSourceSheet;
            ...
            object vRangeDestination = pPrinting.XLGetRange(mCopyLineSUM, mCopy_StartCol, vPasteEndRow, mCopy_EndCol);
            pPrinting.XLCopyRange(vRangeSource, vRangeDestination);  // 복사.
            mCopyLineSUM = vPasteEndRow;

            return mCopyLineSUM; 
        }

        private void CopyAndPaste_Total(XL.XLPrint pPrinting, int pCurrentRow)
        {
            int vPasteEndRow = pCurrentRow + mCopy_EndRow;
            string vActiveSheet = mSourceSheet3;
            ...
            //엑셀 쉬트에서 복사 시작할 열번호, 엑셀 쉬트에서 복사 종료할 행번호, 엑셀 쉬트에서 복사 종료할 열번호
            pPrinting.XLActiveSheet(mTargetSheet);
            object vRangeDestination = pPrinting.XLGetRange(pCurrentRow, mCopy_StartCol, vPasteEndRow, mCopy_EndCol);
            pPrinting.XLCopyRange(vRangeSource, vRangeDestination);  // 복사.
