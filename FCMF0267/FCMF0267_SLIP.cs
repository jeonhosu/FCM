...
namespace FCMF0267
{
    public partial class FCMF0267_SLIP : Office2007Form
    {
        #region ----- Variables -----
        ...
        public FCMF0267_SLIP()
        {
            InitializeComponent();
        }

        public FCMF0267_SLIP(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        public FCMF0267_SLIP(Form pMainForm, ISAppInterface pAppInterface, string pStatus, Object pSESSION_ID, Object pSYS_DATE)
        {
            InitializeComponent();
            isAppInterfaceAdv1.AppInterface = pAppInterface;

            mSlip_Status = pStatus;
        ...
        {
            string vAPPROVAL_NUM =  iConv.ISNull(IGR_CARD_SLIP_GROUP_APPR.GetCellValue("APPROVAL_NUM"));
            int vIDX_APPROVAL_NUM = IGR_CARD_SLIP_GROUP_APPR.GetColumnToIndex("APPROVAL_NUM");
            int vIDX_ACC_INPUT_YN = IGR_CARD_SLIP_GROUP_APPR.GetColumnToIndex("ACC_INPUT_YN");

            try
            {                 
                IDA_CARD_SLIP_GROUP_ACQ.SetSelectParamValue("W_SYS_DATE", mSys_Date);
                IDA_CARD_SLIP_GROUP_ACQ.SetSelectParamValue("W_SESSION_ID", mSession_ID);
                IDA_CARD_SLIP_GROUP_ACQ.SetSelectParamValue("W_SLIP_FLAG", mSlip_Status);
                IDA_CARD_SLIP_GROUP_ACQ.Fill();
            }
            catch (Exception ex)
            {
            ...
            //CURRENT POSITION//
            for(int r =0; r < IGR_CARD_SLIP_GROUP_APPR.RowCount;r++)
            {
                if(vAPPROVAL_NUM == iConv.ISNull(IGR_CARD_SLIP_GROUP_APPR.GetCellValue(r, vIDX_APPROVAL_NUM)))
                {
                    IGR_CARD_SLIP_GROUP_APPR.CurrentCellMoveTo(r, vIDX_ACC_INPUT_YN);
                    IGR_CARD_SLIP_GROUP_APPR.CurrentCellActivate(r, vIDX_ACC_INPUT_YN);
        ...
        private void Init_Sub_Panel(bool pShow_Flag, string pSub_Panel)
        {
            ...
            if (pShow_Flag == true)
            {
                try
                {
                    ...
                    {
                        IDC_GET_CARD_VAT.SetCommandParamValue("W_SYS_DATE", mSys_Date);
                        IDC_GET_CARD_VAT.SetCommandParamValue("W_SESSION_ID", mSession_ID);
                        IDC_GET_CARD_VAT.SetCommandParamValue("W_CARD_ACQUIRE_ID", IGR_CARD_SLIP_GROUP_APPR.GetCellValue("CARD_ACQUIRE_ID"));
                        IDC_GET_CARD_VAT.ExecuteNonQuery();
                        S_CARD_ACQUIRE_ID.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_CARD_ACQUIRE_ID");
                        S_CARD_NUM.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_CARD_NUM");
                        S_VAT_TAX_TYPE.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_VAT_TAX_TYPE");
                        S_VAT_TAX_TYPE_NAME.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_VAT_TAX_TYPE_NAME");
                        S_VAT_VENDOR_ID.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_VAT_VENDOR_ID");
                        S_VAT_VENDOR_CODE.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_VAT_VENDOR_CODE");
                        S_VAT_VENDOR_NAME.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_VAT_VENDOR_NAME");
                        S_VAT_SUPPLY_AMOUNT.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_VAT_SUPPLY_AMOUNT");
                        S_VAT_AMOUNT.EditValue = IDC_GET_CARD_VAT.GetCommandParamValue("O_VAT_AMOUNT"); 
                        ...
                        GB_VAT.BringToFront();
                        GB_VAT.Border3DStyle = Border3DStyle.Bump;
                        GB_VAT.BorderStyle = BorderStyle.Fixed3D;
            ...
            else
            {
                try
                {
                    if (pSub_Panel == "ALL")
                    {
                        GB_VAT.Visible = false; 
                    }
                    else if (pSub_Panel == "AP_VAT")
                    {
                        GB_VAT.Visible = false;

                        Init_Total_GL_Amount();     //합계금액 동기화//
                    } 
        ...
        private Boolean Check_SlipHeader_Added()
        {
            ...
            //헤더 체크 
            for (int r = 0; r < IDA_CARD_SLIP_GROUP_ACQ.SelectRows.Count; r++)
            {
                if (IDA_CARD_SLIP_GROUP_ACQ.SelectRows[r].RowState == DataRowState.Added ||
                    IDA_CARD_SLIP_GROUP_ACQ.SelectRows[r].RowState == DataRowState.Modified)
                {
        ...
        private void InsertSlipLine()
        {
            ...
            REMARK.EditValue = HEADER_REMARK.EditValue;
        ...
        private void Sync_BTN_VAT(string pEnabled_Flag)
        {
            //BTN//
            if (pEnabled_Flag == "Y")
            {
                BTN_VAT.Enabled = true; 
            }
            else
            {
                BTN_VAT.Enabled = false;
            }
        }
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Update)
                {
                    ...
                    IDA_CARD_SLIP_GROUP_ACQ.Update(); 
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Cancel)
                {
                    mStatus = "QUERY";
                    if (IDA_CARD_SLIP_GROUP_ACQ.IsFocused)
                    {
                        IDA_SLIP_LINE.Cancel();
                        IDA_CARD_SLIP_GROUP_ACQ.Cancel();
                    }
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Delete)
                {
                    if (IDA_CARD_SLIP_GROUP_ACQ.IsFocused)
                    {
                        ...
                        IDA_CARD_SLIP_GROUP_ACQ.Delete();
                    }
                    else if (IDA_SLIP_LINE.IsFocused)
                    {
                        IDA_SLIP_LINE.Delete();
        ...
        private void Save_Slip_Line()
        {
            IDA_CARD_SLIP_GROUP_ACQ.Update();
        }

        private void Delete_Slip_Line()
        {
            IDA_SLIP_LINE.Delete();
        }
        ...
        private void FCMF0267_SLIP_Load(object sender, EventArgs e)
        {           
            // 회계장부 정보 설정.
            GetAccountBook();
            ...
            IDA_CARD_SLIP_GROUP_ACQ.FillSchema();
        }

        private void FCMF0267_SLIP_Shown(object sender, EventArgs e)
        {
            BTN_LINE_INSERT.BringToFront();
            BTN_LINE_DELETE.BringToFront();
            BTN_SAVE.BringToFront();
        ...
        private void ibtSUB_FORM_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            {
                S_VAT_VENDOR_CODE.EditValue = Get_Management_Value("SUPPLY_AMOUNT");   //공급가액 설정.
        ...
        private void S_VAT_SUPPLY_AMOUNT_CurrentEditValidating(object pSender, ISEditAdvValidatingEventArgs e)
        {
            IDC_VAT_AMT_P.SetCommandParamValue("W_VAT_TAX_TYPE", S_VAT_TAX_TYPE.EditValue);
            IDC_VAT_AMT_P.SetCommandParamValue("W_SUPPLY_AMT", S_VAT_SUPPLY_AMOUNT.EditValue);
            IDC_VAT_AMT_P.ExecuteNonQuery();
            S_VAT_AMOUNT.EditValue = IDC_VAT_AMT_P.GetCommandParamValue("O_VAT_AMT");
        ...
        private void BTN_VAT_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            if(VAT_FLAG.CheckedState == ISUtil.Enum.CheckedState.Unchecked)
            {
                return;
            }
        ...
        private void S_BTN_OK_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            IDC_VAT_AMT_P.SetCommandParamValue("W_SUPPLY_AMT", S_VAT_SUPPLY_AMOUNT.EditValue);
            IDC_VAT_AMT_P.ExecuteNonQuery();
            decimal vVat_Amount = iConv.ISDecimaltoZero(IDC_VAT_AMT_P.GetCommandParamValue("O_VAT_AMT"));
            if(Math.Truncate(iConv.ISDecimaltoZero(S_VAT_AMOUNT.EditValue) / 10) != Math.Truncate(vVat_Amount / 10))
            {
                if (MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10633"), "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    return;
                }
            }

            IDC_SAVE_CARD_VAT.ExecuteNonQuery();
            string vSTATUS = iConv.ISNull(IDC_SAVE_CARD_VAT.GetCommandParamValue("O_STATUS"));
            string vMESSAGE = iConv.ISNull(IDC_SAVE_CARD_VAT.GetCommandParamValue("O_MESSAGE"));
            if(vSTATUS == "F")
            {
                MessageBoxAdv.Show(vMESSAGE, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
        ...
        private void ILA_VENDOR_CODE_RefreshLookupData(object pSender, ISRefreshLookupDataEventArgs e)
        {
            ILD_VAT_VENDOR_CODE.SetLookupParamValue("W_USE_DATE_FR", SLIP_DATE.EditValue);
            ILD_VAT_VENDOR_CODE.SetLookupParamValue("W_USE_DATE_TO", SLIP_DATE.EditValue);  
        }
        ...
        private void IDA_CARD_SLIP_GROUP_APPR_NewRowMoved(object pSender, ISBindingEventArgs pBindingManager)
        {
            ...
            Sync_BTN_VAT(iConv.ISNull(pBindingManager.DataRow["VAT_FLAG"]));
        ...
        private void idaSLIP_LINE_PreRowUpdate(ISPreRowUpdateEventArgs e)
        {
            ...
            {// 적요
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_90004", string.Format("{0}{1}", "&&FIELD_NAME:=", Get_Edit_Prompt(REMARK))), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
                return;
            }
        ...
        private void IDA_CARD_SLIP_GROUP_APPR_UpdateCompleted(object pSender)
        {
            Search_DB();
        }
