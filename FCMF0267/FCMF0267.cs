...
namespace FCMF0267
{
    public partial class FCMF0267 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public FCMF0267(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void Search_DB()
        {
            ...
            IGR_CREDIT_ACQUIRE.LastConfirmChanges();
            IDA_CARD_ACQUIRE.OraSelectData.AcceptChanges();
            IDA_CARD_ACQUIRE.Refillable = true;
            IGR_CREDIT_ACQUIRE.ResetDraw = true;

            string vAPPR_NUM = iConv.ISNull(IGR_CREDIT_ACQUIRE.GetCellValue("APPROVAL_NUM"));
            int vCOL_IDX = IGR_CREDIT_ACQUIRE.GetColumnToIndex("APPROVAL_NUM");
            IDA_CARD_ACQUIRE.Fill();
            Sum_Amount();

            if (iConv.ISNull(vAPPR_NUM) != string.Empty)
            {
                for (int i = 0; i < IGR_CREDIT_ACQUIRE.RowCount; i++)
                {
                    if (vAPPR_NUM == iConv.ISNull(IGR_CREDIT_ACQUIRE.GetCellValue(i, vCOL_IDX)))
                    {
                        IGR_CREDIT_ACQUIRE.CurrentCellMoveTo(i, vCOL_IDX);
                        IGR_CREDIT_ACQUIRE.CurrentCellActivate(i, vCOL_IDX);
                        return;
                    }
                }
            }
        ...
        private void Sum_Amount()
        {
            ...
            foreach (DataRow vROW in IDA_CARD_ACQUIRE.CurrentRows)
            {
                //IDA_CARD_ACQUIRE
        ...
        {
            int vIDX_SELECT_YN = IGR_CREDIT_ACQUIRE.GetColumnToIndex("SELECT_YN");
            int vIDX_SLIP_DATE = IGR_CREDIT_ACQUIRE.GetColumnToIndex("SLIP_DATE");
            int vIDX_USE_PERSON_NAME = IGR_CREDIT_ACQUIRE.GetColumnToIndex("USE_PERSON_NAME");
            int vIDX_BUDGET_DEPT_NAME = IGR_CREDIT_ACQUIRE.GetColumnToIndex("BUDGET_DEPT_NAME");
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    IDA_CARD_ACQUIRE.Update();
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Cancel)
                {
                    if (IDA_CARD_ACQUIRE.IsFocused)
                    {
                        IDA_CARD_ACQUIRE.Cancel();
                ...
                {
                    mSlip_Header_ID = IGR_CREDIT_ACQUIRE.GetCellValue("SLIP_HEADER_ID");
                    mSlip_Date = IGR_CREDIT_ACQUIRE.GetCellValue("SLIP_DATE");
                    mSlip_Num = IGR_CREDIT_ACQUIRE.GetCellValue("SLIP_NUM");
        ...
        private void FCMF0267_Load(object sender, EventArgs e)
        {
        ...
        private void FCMF0267_Shown(object sender, EventArgs e)
        {
            IDA_CARD_ACQUIRE.FillSchema();
            W_PURCHASE_FLAG.CheckedState = ISUtil.Enum.CheckedState.Checked;
            W_DATE_TYPE.EditValue = W_PURCHASE_FLAG.RadioCheckedString;

            //회계 장부//
            GetAccountBook(); 
        }

        private void W_TRX_FLAG_Click(object sender, EventArgs e)
        {
            if(W_TRX_FLAG.CheckedState == ISUtil.Enum.CheckedState.Checked)
            {
                W_DATE_TYPE.EditValue = W_TRX_FLAG.RadioCheckedString;
            }
        }

        private void W_PURCHASE_FLAG_Click(object sender, EventArgs e)
        {
            if(W_PURCHASE_FLAG.CheckedState == ISUtil.Enum.CheckedState.Checked)
            {
                W_DATE_TYPE.EditValue = W_PURCHASE_FLAG.RadioCheckedString;
            }
        }
        ...
        {                          
            ...
            int vIDX_SELECT_YN = IGR_CREDIT_ACQUIRE.GetColumnToIndex("SELECT_YN");
            int vIDX_CARD_ACQUIRE_ID = IGR_CREDIT_ACQUIRE.GetColumnToIndex("CARD_ACQUIRE_ID");
            int vIDX_CARD_NUM = IGR_CREDIT_ACQUIRE.GetColumnToIndex("CARD_NUM");
            int vIDX_APPROVAL_NUM = IGR_CREDIT_ACQUIRE.GetColumnToIndex("APPROVAL_NUM");
             
            for (int r = 0; r < IGR_CREDIT_ACQUIRE.RowCount; r++)
            {
                if ("Y" == iConv.ISNull(IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_SELECT_YN)))
                {
                    vREC_CNT++;
                    IGR_CREDIT_ACQUIRE.CurrentCellMoveTo(r, vIDX_SELECT_YN);
                    IGR_CREDIT_ACQUIRE.CurrentCellActivate(r, vIDX_SELECT_YN);
                    ...
                    //해당 체크박스는 숨김 처리
                    IDC_SAVE_CARD_SLIP_GROUP.SetCommandParamValue("P_CARD_ACQUIRE_ID", IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_CARD_ACQUIRE_ID));
                    IDC_SAVE_CARD_SLIP_GROUP.SetCommandParamValue("P_SYS_DATE", vSYS_DATE);
                    IDC_SAVE_CARD_SLIP_GROUP.SetCommandParamValue("P_CARD_NUM", IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_CARD_NUM));
                    IDC_SAVE_CARD_SLIP_GROUP.SetCommandParamValue("P_APPROVAL_NUM", IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_APPROVAL_NUM));
            ...
            //전표 생성 화면//
            FCMF0267_SLIP vFCMF0267_SLIP = new FCMF0267_SLIP(this.MdiParent, isAppInterfaceAdv1.AppInterface, iConv.ISNull(W_SLIP_IF_FLAG.EditValue), mSession_ID, vSYS_DATE);
            DialogResult dlgResult = vFCMF0267_SLIP.ShowDialog(); 

            mSlip_Header_ID = vFCMF0267_SLIP.Get_Slip_Header_ID;
            mSlip_Num = vFCMF0267_SLIP.Get_Slip_Num;
            mSlip_Date = vFCMF0267_SLIP.Get_Slip_Date;

            vFCMF0267_SLIP.Close();
        ...
        private void BTN_CANCEL_SLIP_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            for (int r = 0; r < IGR_CREDIT_ACQUIRE.RowCount; r++)
            {
                if ("Y" == iConv.ISNull(IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_SELECT_YN)))
                {
                    ...
                    if (iConv.ISNull(IGR_CREDIT_ACQUIRE.GetCellValue("SLIP_HEADER_ID")) == string.Empty)
                    {
                        Application.UseWaitCursor = false;
                        System.Windows.Forms.Cursor.Current = Cursors.Default;
                        Application.DoEvents();
                    ...
                    //전표 일괄생성 패키지에서 처리하므로 폼에서는 제어할수 없음
                    //해당 체크박스는 숨김 처리
                    IDC_SAVE_CANCEL_CARD_SLIP.SetCommandParamValue("P_CARD_ACQUIRE_ID", IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_CARD_ACQUIRE_ID));
                    IDC_SAVE_CANCEL_CARD_SLIP.SetCommandParamValue("P_SYS_DATE", vSYS_DATE);
                    IDC_SAVE_CANCEL_CARD_SLIP.SetCommandParamValue("P_CARD_NUM", IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_CARD_NUM));
                    IDC_SAVE_CANCEL_CARD_SLIP.SetCommandParamValue("P_APPROVAL_NUM", IGR_CREDIT_ACQUIRE.GetCellValue(r, vIDX_APPROVAL_NUM));
        ...
        {
            if(IGR_CREDIT_ACQUIRE.RowCount < 0)
            {
                return;
            }
            ...
            object vCARD_ACQUIRE_ID = IGR_CREDIT_ACQUIRE.GetCellValue("CARD_ACQUIRE_ID");
            IDC_DELETE_CARD_ACQUIRE_SLIP.SetCommandParamValue("W_CARD_ACQUIRE_ID", vCARD_ACQUIRE_ID);
            IDC_DELETE_CARD_ACQUIRE_SLIP.ExecuteNonQuery();
            string vSTATUS = iConv.ISNull(IDC_DELETE_CARD_ACQUIRE_SLIP.GetCommandParamValue("O_STATUS"));
            string vMESSAGE = iConv.ISNull(IDC_DELETE_CARD_ACQUIRE_SLIP.GetCommandParamValue("O_MESSAGE"));
            if (IDC_DELETE_CARD_ACQUIRE_SLIP.ExcuteError)
            {
                Application.UseWaitCursor = false;
                System.Windows.Forms.Cursor.Current = Cursors.Default;
                Application.DoEvents();

                MessageBoxAdv.Show(IDC_DELETE_CARD_ACQUIRE_SLIP.ExcuteErrorMsg, "Slip Delete-Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else if (vSTATUS == "F")
            {
        ...
        private void ILA_DEPT_RefreshLookupData(object pSender, ISRefreshLookupDataEventArgs e)
        {
            ...
            ILD_BUDGET_USE_DEPT.SetLookupParamValue("W_EFFECTIVE_DATE_FR", IGR_CREDIT_ACQUIRE.GetCellValue("SLIP_DATE"));
            ILD_BUDGET_USE_DEPT.SetLookupParamValue("W_EFFECTIVE_DATE_TO", IGR_CREDIT_ACQUIRE.GetCellValue("SLIP_DATE")); 
        }
        ...
        {
            if (iConv.ISNull(e.Row["CARD_ACQUIRE_ID"]) == string.Empty)
            {
                e.Cancel = true;
        ...
        {
            int vIDX_EXCHANGE_RATE = IGR_CREDIT_ACQUIRE.GetColumnToIndex("EXCHANGE_RATE");
            object vUpdatable = 0;
            ...
            IGR_CREDIT_ACQUIRE.GridAdvExColElement[vIDX_EXCHANGE_RATE].Insertable = vUpdatable;
            IGR_CREDIT_ACQUIRE.GridAdvExColElement[vIDX_EXCHANGE_RATE].Updatable = vUpdatable;
