...
namespace FCMF0992
{
    public partial class FCMF0992_FIX : Office2007Form
    {
        ...
        {
            //검증//
            if (iConv.ISNull(SRC_TAX_BILL_ISSUE_NO.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_10183"), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                SRC_TAX_BILL_ISSUE_NO.Focus();
                return;
            }
            if (iConv.ISNull(TAX_BILL_FIX_TYPE.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("EAPP_10184"), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                TAX_BILL_FIX_TYPE.Focus();
                return;
            }
            ...
            IDC_SET_FIX_BILL365.DataTransaction = isDataTransaction1;
            ...
            IDC_SET_FIX_BILL365.SetCommandParamValue("W_TAX_BILL_ISSUE_NO", SRC_TAX_BILL_ISSUE_NO.EditValue);
            IDC_SET_FIX_BILL365.ExecuteNonQuery();
            vSTATUS = iConv.ISNull(IDC_SET_FIX_BILL365.GetCommandParamValue("O_STATUS"));
            vMESSAGE = iConv.ISNull(IDC_SET_FIX_BILL365.GetCommandParamValue("O_MESSAGE"));
            if (vSTATUS == "F")
            {
            ...
            IDC_SET_FIX_BILL365.DataTransaction = null;
            Application.UseWaitCursor = false;
            System.Windows.Forms.Cursor.Current = Cursors.Default;
            Application.DoEvents();
