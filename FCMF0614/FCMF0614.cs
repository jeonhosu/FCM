...
namespace FCMF0614
{
    public partial class FCMF0614 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public FCMF0614(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        {
            if (iConv.ISNull(BUDGET_PERIOD_0.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10022"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                BUDGET_PERIOD_0.Focus();
                return;
            }

            IDA_BUDGET_ACTUAL_SUM_DEPT.Fill();
            IDA_BUDGET_ACTUAL_SUM_ACCT.Fill();
        ...
        private void Set_Tab_Focus()
        {
            ...
            {
                IGR_BUDGET_ACTUAL_SUM_DEPT.Focus();
            }
        ...
        private void XLPrinting_1(string pOutChoice)
        {
            ...
            int vCountRow = IGR_BUDGET_ACTUAL_SUM_DEPT.RowCount;
            if (vCountRow < 1)
            {
            ...
            try
            {
                // open해야 할 파일명 지정.
                //-------------------------------------------------------------------------------------
                xlPrinting.OpenFileNameExcel = "FCMF0614_001.xls";
                //-------------------------------------------------------------------------------------
                // 파일 오픈.
                //-------------------------------------------------------------------------------------
                ...
                {
                    //헤더 데이터 설정
                    object vBUDGET_YEAR = iConv.ISNull(BUDGET_PERIOD_0.EditValue).Substring(0, 4);
                    object vBUDGET_MONTH = iConv.ISNull(BUDGET_PERIOD_0.EditValue).Substring(5, 2);

                    //헤더 인쇄
                    xlPrinting.HeaderWrite_1(vBUDGET_YEAR, vBUDGET_MONTH);
                    //라인 인쇄
                    vPageNumber = xlPrinting.LineWrite_1(IGR_BUDGET_ACTUAL_SUM_DEPT);
        ...
        private void XLPrinting_2(string pOutChoice)
        {
            ...
            int vCountRow = IGR_BUDGET_ACTUAL_SUM_ACCT.RowCount;
            if (vCountRow < 1)
            {
                vMessageText = string.Format("Without Data");
                isAppInterfaceAdv1.OnAppMessage(vMessageText);
            ...
            try
            {
                // open해야 할 파일명 지정.
                //-------------------------------------------------------------------------------------
                xlPrinting.OpenFileNameExcel = "FCMF0614_002.xls";
                //-------------------------------------------------------------------------------------
                // 파일 오픈.
                //-------------------------------------------------------------------------------------
                bool isOpen = xlPrinting.XLFileOpen();
                ...
                if (isOpen == true)
                {
                    ...
                    //헤더 인쇄
                    xlPrinting.HeaderWrite_2(vBUDGET_YEAR, vBUDGET_MONTH);
                    //라인 인쇄
                    vPageNumber = xlPrinting.LineWrite_2(IGR_BUDGET_ACTUAL_SUM_ACCT);
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    if (IDA_BUDGET_ACTUAL_SUM_DEPT.IsFocused)
                    {
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Cancel)
                {
                    if (IDA_BUDGET_ACTUAL_SUM_DEPT.IsFocused)
                    {
                        IDA_BUDGET_ACTUAL_SUM_DEPT.Cancel();
                    }
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Delete)
                {
                    if (IDA_BUDGET_ACTUAL_SUM_DEPT.IsFocused)
                    {
                        IDA_BUDGET_ACTUAL_SUM_DEPT.Delete();
                    }
                }
        ...
        private void FCMF0614_Load(object sender, EventArgs e)
        {

        }

        private void FCMF0614_Shown(object sender, EventArgs e)
        {
            BUDGET_PERIOD_0.EditValue = iDate.ISYearMonth(DateTime.Today);
        ...
        private void ilaDEPT_FR_0_PrePopupShow(object pSender, ISLookupPopupShowEventArgs e)
        {
            ...
            ildDEPT_FR_TO.SetLookupParamValue("W_EFFECTIVE_DATE_FR", iDate.ISMonth_1st(BUDGET_PERIOD_0.EditValue));
            ildDEPT_FR_TO.SetLookupParamValue("W_EFFECTIVE_DATE_TO", iDate.ISMonth_Last(BUDGET_PERIOD_0.EditValue));
        }
