...
namespace FCMF0611
{
    public class XLPrinting
    {
        ...
        private int XLLine_1(System.Data.DataRow pRow, int pXLine, int pDeptRow, bool pPrint_Flag)
        {
            ...
            { // 원본을 복사해서 타겟 에 복사해 넣음.(
                ...
                vXLColumn = 6;
                mPrinting.XLSetCell(pDeptRow, vXLColumn, vConvertString);
        ...
        private int XLLine_2(System.Data.DataRow pRow, int pXLine, int pAccountRow)
        {
            // pGridRow : 그리드의 현재 읽는 행, pXLine : 엑셀의 인쇄해야 하는 행. pGDColumn : 그리드 위치, pXLColumn : 엑셀 위치.
            int vXLine = pXLine; //엑셀에 내용이 표시되는 행 번호
            int vXLColumn = 0;
        ...
        public int LineWrite_1(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter)
        {// 실제 호출되는 부분.
            mPageNumber = 0;
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    foreach (System.Data.DataRow vRow in pAdapter.OraSelectData.Rows)
                    {
                        ...
                        if (vDEPT_CODE == null || vDEPT_CODE == string.Empty)
                        {

                        }
                        else if (vDEPT_CODE != iString.ISNull(vRow["DEPT_CODE"], "-"))
                        {
                            ...
                            if (mIsNewPage == true)
                            {
                                vDeptRow = vDeptRow + mCopy_EndRow;
                        ...
                        //계정코드 동일 여부 체크.
                        vPrint_Flag = true;
                        if (vACCOUNT_GROUP_CODE == null || vACCOUNT_GROUP_CODE == string.Empty || mIsNewPage == true)
                        {
                        ...
                        {
                            vPrint_Flag = false;
                            mPrinting.XL_LineClearTOP(mCurrentRow, 1, 7);
                        }
                        vACCOUNT_GROUP_CODE = iString.ISNull(vRow["ACCOUNT_GROUP_CODE"]);

                        mCurrentRow = XLLine_1(vRow, mCurrentRow, vDeptRow, vPrint_Flag); // 현재 위치 인쇄 후 다음 인쇄행 리턴.
                        ...
                        vDEPT_CODE = iString.ISNull(vRow["DEPT_CODE"]);
                    }
                }
                #endregion;
            }
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    foreach (System.Data.DataRow vRow in pAdapter.OraSelectData.Rows)
                    {
                        ...
                        if (vACCOUNT_CODE == null || vACCOUNT_CODE == string.Empty)
                        {
                        ...
                        mCurrentRow = XLLine_2(vRow, mCurrentRow, vAccountRow); // 현재 위치 인쇄 후 다음 인쇄행 리턴.
                        vPageRowCount = vPageRowCount + 1;
                        ...
                        else
                        {
                            ...
                            if (mIsNewPage == true)
                            {
                                vAccountRow = vAccountRow + vAccountRow;
