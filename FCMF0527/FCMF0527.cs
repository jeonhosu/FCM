...
namespace FCMF0527
{
    public partial class FCMF0527 : Office2007Form
    {
        #region ----- Variables -----
        ...
        public FCMF0527(Form pMainForm, ISAppInterface pAppInterface)
        {
            InitializeComponent();
        ...
        private void GetAccountBook()
        {
            ...
            //적요 복사 여부//
            IDC_SLIP_REMARK_COPY_P.SetCommandParamValue("W_ASSEMBLY_ID", "FCMF0202");
        ...
        private void Set_Insert_Batch_Line()
        {
            IDA_BATCH_BILL_SLIP.Fill();
            if (IDA_BATCH_BILL_SLIP.SelectRows.Count < 0)
            {
            ...
            try
            {
                ...
                for (int i = 0; i < IDA_BATCH_BILL_SLIP.SelectRows.Count; i++)
                {
                    ...
                    for (int c = 0; c < igrSLIP_LINE.GridAdvExColElement.Count; c++)
                    {
                        ...
                        {
                            igrSLIP_LINE.SetCellValue(i + Row_Count, c, IDA_BATCH_BILL_SLIP.OraDataSet().Rows[i][c]);
                        }
                    }
                }
                ...
                IDC_BATCH_BILL_STATUS.ExecuteNonQuery();
                vSTATUS = iString.ISNull(IDC_BATCH_BILL_STATUS.GetCommandParamValue("O_STATUS"));
                vMESSAGE = iString.ISNull(IDC_BATCH_BILL_STATUS.GetCommandParamValue("O_MESSAGE"));
        ...
        private void Set_Slip_Line_Seq()
        {
            ...
            try
            {
                ...
                //현재 다음 line seq                 
                if ((IDA_BATCH_LINE.CurrentRowPosition() + 1) == IDA_BATCH_LINE.CurrentRows.Count)
                {
                    vNext_Line_Seq = 0;
                }
                else
                {
                    int mNextRowPosition = IDA_BATCH_LINE.CurrentRowPosition() + 1;
        ...
        private void Init_Budget_Dept()
        {
            ...
            {//budget dept
                ...
                mPrevious_Code = IDA_BATCH_LINE.CurrentRows[mPreviousRowPosition]["BUDGET_DEPT_CODE"];
            ...
            else
            {
                ...
                BUDGET_DEPT_CODE_L.EditValue = DEPT_CODE.EditValue;
        ...
        private void XLPrinting_Main(string pOutput_Type)
        {
            ...
            AssmblyRun_Manual("FCMF0202", vSlip_Header_id, vGL_Date, vGL_Num);
        ...
        private void XLPrinting1(string pOutput_Type)
        {
                ...
                (TB_BATCH.SelectedIndex == 1 && iString.ISNull(BATCH_HEADER_ID.EditValue) != string.Empty))
            {
                ...
                {
                    //-------------------------------------------------------------------------------------
                    xlPrinting.OpenFileNameExcel = "FCMF0527_001.xls";
                    //-------------------------------------------------------------------------------------
                    ...
                    if (isOpen == true)
                    {
                        ...
                        idaPRINT_SLIP_LINE.SetSelectParamValue("W_BATCH_HEADER_ID", vObject);
        ...
        private void FCMF0527_Load(object sender, EventArgs e)
        {
        ...
        private void FCMF0527_Shown(object sender, EventArgs e)
        {
            ibtSUB_FORM.Visible = false;
        ...
        private void btnSET_BATCH_LIST_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            FCMF0527_SET vFCMF0527_SET = new FCMF0527_SET(isAppInterfaceAdv1.AppInterface, GL_DATE.EditValue);
            mEAPF1102.SetProperties(EAPF1102.INIT_TYPE.None, vFCMF0527_SET, isAppInterfaceAdv1.AppInterface);
            vRESULT = vFCMF0527_SET.ShowDialog();
            if (vRESULT == DialogResult.OK)
            {
                object vDUE_DATE = vFCMF0527_SET.Get_Due_Date;
                                
                IDA_BATCH_BILL_SLIP.SetSelectParamValue("P_DUE_DATE", vDUE_DATE);
                if (iString.ISNull(ACCOUNT_CONTROL_ID.EditValue) == string.Empty)
                {
                    IDA_BATCH_LINE.Delete();
                }
                IDA_BATCH_LINE.MoveLast(igrSLIP_LINE.Name);
                Set_Insert_Batch_Line();
                Init_Currency_Code("Y");
                Init_Currency_Amount();
            }
            vFCMF0527_SET.Dispose();
        ...
        private void ibtSUB_FORM_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            if (mCurrency_Code.ToString() != igrSLIP_LINE.GetCellValue("CURRENCY_CODE").ToString()
                  && iString.ISDecimaltoZero(igrSLIP_LINE.GetCellValue("GL_CURR_AMOUNT")) == Convert.ToInt32(0))
            ...
            {//받을어음
                ...
                FCMF0527_BILL vFCMF0527_BILL = new FCMF0527_BILL(isAppInterfaceAdv1.AppInterface, mDEPT_ID, mDEPT_NAME
                ...
                mEAPF1102.SetProperties(EAPF1102.INIT_TYPE.None, vFCMF0527_BILL, isAppInterfaceAdv1.AppInterface);
                dlgResult = vFCMF0527_BILL.ShowDialog();
                if (dlgResult == DialogResult.OK)
                {
                    //어음금액
                    GL_AMOUNT.EditValue = vFCMF0527_BILL.Get_BILL_AMOUNT;
                    //거래처.
                    Set_Management_Value("CUSTOMER", vFCMF0527_BILL.Get_VENDOR_CODE, vFCMF0527_BILL.Get_VENDOR_NAME);
                    //은행
                    Set_Management_Value("BANK", vFCMF0527_BILL.Get_BANK_CODE, vFCMF0527_BILL.Get_BANK_NAME);
                    //세금계산서발행일
                    Set_Management_Value("VAT_ISSUE_DATE", vFCMF0527_BILL.Get_VAT_ISSUE_DATE, null);
                    //발행일자
                    Set_Management_Value("ISSUE_DATE", vFCMF0527_BILL.Get_ISSUE_DATE, null);
                    //만기일자
                    Set_Management_Value("DUE_DATE", vFCMF0527_BILL.Get_DUE_DATE, null);
                    //만기일자
                    Set_Management_Value("TR_EXPIRATION_DATE", vFCMF0527_BILL.Get_DUE_DATE, null);  
                    //어음번호.
                    Set_Management_Value("RECEIVABLE_BILL", vFCMF0527_BILL.Get_BILL_NUM, String.Format("{0:###,###,###,###,###,###}", vFCMF0527_BILL.Get_BILL_AMOUNT));

                    Init_DR_CR_Amount();    // 차대금액 생성 //
                    Init_Total_GL_Amount(); // 총합계 및 분개 차액 생성 //
                }
                vFCMF0527_BILL.Dispose();
            }
            ...
            {//지급어음
                ...
                if (dlgResult == DialogResult.OK)
                {
                    ...
                    Set_Management_Value("TR_EXPIRATION_DATE", vFCMF0527_BILL.Get_DUE_DATE, null);  
                    //어음번호.
                    Set_Management_Value("PAYABLE_BILL", vFCMF0527_BILL.Get_BILL_NUM, String.Format("{0:###,###,###,###,###,###}", vFCMF0527_BILL.Get_BILL_AMOUNT));
            ...
            //    object mVAT_AMOUNT = REFER8.EditValue;     //세액 설정.
            //    object mCOUNT = 0;                          // 매수.

            //    FCMF0527_ASSET vFCMF0527_ASSET = new FCMF0527_ASSET(isAppInterfaceAdv1.AppInterface, mSLIP_LINE_ID, mSUPPLY_AMOUNT, mVAT_AMOUNT);
            //    Application.UseWaitCursor = true;
            //    dlgResult = vFCMF0527_ASSET.ShowDialog();
            //    //if (dlgResultValue == DialogResult.OK)
            //    //{
            //    //    // 건물등 감가상각 취득금액 표시.
            //    //    mCOUNT = vFCMF0203_ASSET.Get_ASSET_COUNT;
            //    //    if (iString.ISDecimaltoZero(mCOUNT, 0) == 0)
            //    //    {
            //    //        REFER10.EditValue = DBNull.Value;
            //    //    }
            //    //    else
            //    //    {
            //    //        REFER10.EditValue = vFCMF0203_ASSET.Get_SUP_AMOUNT;
            //    //    }
            ...
            //    FCMF0527_ITEM_DEAL vFCMF0527_ITEM_DEAL = new FCMF0527_ITEM_DEAL(isAppInterfaceAdv1.AppInterface, mISSUE_NUM, mCURRENCY_CODE
            //                                                                    , mVENDOR_CODE, mBANK_CODE, mISSUE_DATE);

            //    dlgResult = vFCMF0527_ITEM_DEAL.ShowDialog();
            //    if (dlgResult == DialogResult.OK)
            //    {
            //        //거래처.4
            //    Set_Management_Value("CUSTOMER", vFCMF0527_ITEM_DEAL.Get_VENDOR_CODE, vFCMF0527_ITEM_DEAL.Get_VENDOR_NAME);

            //        //구매(공급)확인번호
            //    Set_Management_Value("PC_ISSUE_NO", vFCMF0527_ITEM_DEAL.Get_ISSUE_NUM, DBNull.Value);

            //    Set_Management_Value("BANK", vFCMF0527_ITEM_DEAL.Get_BANK_CODE, vFCMF0527_ITEM_DEAL.Get_BANK_NAME);

            //    Set_Management_Value("ISSUE_DATE", vFCMF0527_ITEM_DEAL.Get_ISSUE_DATE, DBNull.Value);
            //    }
            //    vFCMF0527_ITEM_DEAL.Dispose();
            //    }
            //    vFCMF0527_ITEM_DEAL.Dispose();
            //}
            Application.UseWaitCursor = false;
