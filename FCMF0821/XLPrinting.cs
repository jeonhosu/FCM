...
namespace FCMF0821
{
    public class XLPrinting
    {
        ...
        public void HeaderWrite(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, object pPERIOD
                                , object pAPPLY_BASE_DESC, object pSUBMIT_IMPOSSIBLE_DESC
                                , object pSUBMIT_DOCUMENT_DESC, object pSUBMIT_DATE
                                , object pATTACH_CONTENTS_1, object pATTACH_CONTENTS_2, object pATTACH_CONTENTS_3)
        {// 헤더 인쇄.
            int vXLine = 0;
            int vXLColumn = 0;
            ...
            try
            {
                ...
                vXLine = 7;
                vXLColumn = 11;
                mPrinting.XLSetCell(vXLine, vXLColumn, pAPPLY_BASE_DESC);

                vXLine = 7;
                vXLColumn = 36;
                mPrinting.XLSetCell(vXLine, vXLColumn, pSUBMIT_IMPOSSIBLE_DESC);
                                
                vXLine = 8;
                vXLColumn = 11;
                mPrinting.XLSetCell(vXLine, vXLColumn, pSUBMIT_DOCUMENT_DESC);

                vXLine = 8;
                vXLColumn = 36;
                mPrinting.XLSetCell(vXLine, vXLColumn, pSUBMIT_DATE);

                //첨부
                vXLine = 29;
                vXLColumn = 9;
                mPrinting.XLSetCell(vXLine, vXLColumn, pATTACH_CONTENTS_1);

                vXLine = 30;
                vXLColumn = 9;
                mPrinting.XLSetCell(vXLine, vXLColumn, pATTACH_CONTENTS_2);

                vXLine = 31;
                vXLColumn = 9;
                mPrinting.XLSetCell(vXLine, vXLColumn, pATTACH_CONTENTS_3);
            }
            catch (System.Exception ex)
            {
                mMessageError = ex.Message;
        ...
        {// pGridRow : 그리드의 현재 읽는 행, pXLine : 엑셀의 인쇄해야 하는 행. pGDColumn : 그리드 위치, pXLColumn : 엑셀 위치.
            ...
            { // 타겟쉬트 Active.
                ...
                // 원화금액 합계;
                mTOT_AMOUNT = mTOT_AMOUNT + vConvertDecimal;
                //11 - 비고
                ...
                if (vTotalRow > 0)
                {
                    ...
                    for (int vRow = 0; vRow < vTotalRow; vRow++)
                    {
                        ...
                        if (vRow == vTotalRow - 1)
                        {
                            ...
                            mCurrentRow = XLTOTAL_Line(mCurrentRow);     //합계.
                        }
                        else
                        {
