...
namespace FCMF0512
{
    public class XLPrinting09 : XLInterface
    {
        #region ----- Variables -----
        ...
        private string mMessageValue1 = string.Empty; //총합계[EAPP_10045]
        ...
        public XLPrinting09(InfoSummit.Win.ControlAdv.ISAppInterface pAppInterface, InfoSummit.Win.ControlAdv.ISMessageAdapter pMessageAdapter)
        {
        ...
        public XLPrinting09(XL.XLPrint pPrinting, InfoSummit.Win.ControlAdv.ISAppInterface pAppInterface, InfoSummit.Win.ControlAdv.ISMessageAdapter pMessageAdapter)
        {
            mPrinting = pPrinting;
            mAppInterface = pAppInterface;
            mMessageAdapter = pMessageAdapter;
        ...
        private void SetArray(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn, out int[] pXLColumn)
        {
            ...
            pGDColumn[0] = pGrid.GetColumnToIndex("BANK_NAME");             //차입처[은행]
            pGDColumn[1] = pGrid.GetColumnToIndex("LOAN_USE_NAME");         //대출과목
            pGDColumn[2] = pGrid.GetColumnToIndex("LIMIT_AMOUNT");          //한도금액
            pGDColumn[3] = pGrid.GetColumnToIndex("LIMIT_REMAIN_AMOUNT");   //한도잔액
            pGDColumn[4] = pGrid.GetColumnToIndex("BEGIN_AMOUNT");          //전일잔액
            pGDColumn[5] = pGrid.GetColumnToIndex("DR_AMOUNT");             //금일차입
            pGDColumn[6] = pGrid.GetColumnToIndex("CR_AMOUNT");             //금일상환
            pGDColumn[7] = pGrid.GetColumnToIndex("REMAIN_AMOUNT");         //차입금잔액
            pGDColumn[8] = pGrid.GetColumnToIndex("CURRENCY_CODE");         //통화
            pGDColumn[9] = pGrid.GetColumnToIndex("REMAIN_CURR_AMOUNT");    //외화잔액
            pGDColumn[10] = pGrid.GetColumnToIndex("ISSUE_DATE");           //약정일
            pGDColumn[11] = pGrid.GetColumnToIndex("DUE_DATE");             //만기일
            pGDColumn[12] = pGrid.GetColumnToIndex("INTER_RATE");           //이자율

            pXLColumn[0] = 2;    //차입처[은행]
            pXLColumn[1] = 11;   //대출과목
            pXLColumn[2] = 16;   //한도금액
            pXLColumn[3] = 22;   //한도잔액
            pXLColumn[4] = 28;   //전일잔액
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx[] pGrid, string pDate)
        {
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    ...
                    mMessageValue1 = mMessageAdapter.ReturnText("EAPP_10045"); //총합계
                    mPrinting.XLActiveSheet("SourceTab9");
                    ...
                    for (int vRow = 0; vRow < vTotalRow; vRow++)
                    {
                        ...
                        else
                        {
                            ...
                            if (mIsNewPage == true)
                            {
                                ...
                                mtmpString1 = string.Empty; //페이지 넘기난후 차입처명이 계속 동일하게 갈 경우, 헤더라인을 지워버러서, 지워지지 않도록 빈값 줌.
                            }
                        }
                    }
                }
        ...
        private int CopyAndPaste(XL.XLPrint pPrinting, int pCopySumPrintingLine)
        {
            ...
            pPrinting.XLActiveSheet("SourceTab9");
