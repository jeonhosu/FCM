...
namespace FCMF0808
{
    public class XLPrinting
    {
        ...
        private int mPrintingLastRow = 30;  //최종 인쇄 라인.

        private int mCurrentRow = 15;       //현재 인쇄되는 row 위치.
        private int mDefaultPageRow = 7;    // 페이지 증가후 PageCount 기본값.
        private int mCurrentRowAdd = 9;     //페이지 증가후 기본적으로 증가해야 할 row수.
        ...
        {// 그리드의 컬럼에 대한 컬럼인덱스 값 산출
            ...
            // 그리드 or 아답터 위치.
            pGDColumn[0] = 0;
            pGDColumn[1] = pGrid.GetColumnToIndex("DOCUMENT_NUM");
            pGDColumn[2] = pGrid.GetColumnToIndex("SHIPPING_DATE");
            pGDColumn[3] = pGrid.GetColumnToIndex("CURRENCY_CODE");
            pGDColumn[4] = pGrid.GetColumnToIndex("EXCHANGE_RATE");
            pGDColumn[5] = pGrid.GetColumnToIndex("CURR_AMOUNT");
            pGDColumn[6] = pGrid.GetColumnToIndex("BASE_AMOUNT");
        ...
        public void HeaderWrite(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, object pPERIOD, object pISSUE_PERIOD, object pWRITE_DATE)
        {// 헤더 인쇄.
            int vXLine = 0;
            int vXLColumn = 0; 
        ...
        private void XLTOTAL_Line(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid)
        {// pGridRow : 그리드의 현재 읽는 행, pXLine : 엑셀의 인쇄해야 하는 행. pGDColumn : 그리드 위치, pXLColumn : 엑셀 위치.
            ...
            { // 원본을 복사해서 타겟 에 복사해 넣음 
                ...
                {
                    if (iString.ISNull(pGrid.GetCellValue(r, pGrid.GetColumnToIndex("VAT_EXPORT_TYPE"))) == "EXPORT")
                    {
                        vXLine = 11;
                    }
                    else if (iString.ISNull(pGrid.GetCellValue(r, pGrid.GetColumnToIndex("VAT_EXPORT_TYPE"))) == "ETC")
                    {
                        vXLine = 12;
                    }
                    else
                    ...
                    //17 - 건수
                    vXLColumnIndex = 17;
                    vObject = pGrid.GetCellValue(r, pGrid.GetColumnToIndex("EXPORT_COUNT"));
                    IsConvert = IsConvertNumber(vObject, out vConvertDecimal);
                    if (IsConvert == true)
                    {
                        vConvertString = string.Format("{0:###,###,###,###,###,###,###,###}", vConvertDecimal);
                    ...
                    //23 - 외화금액
                    vXLColumnIndex = 23;
                    vObject = pGrid.GetCellValue(r, pGrid.GetColumnToIndex("CURR_AMOUNT"));
                    IsConvert = IsConvertNumber(vObject, out vConvertDecimal);
                    if (IsConvert == true)
                    {
                        vConvertString = string.Format("{0:###,###,###,###,###,###,###,##0.00}", vConvertDecimal);
                    ...
                    //31 - 원화금액
                    vXLColumnIndex = 31;
                    vObject = pGrid.GetCellValue(r, pGrid.GetColumnToIndex("BASE_AMOUNT"));
                    IsConvert = IsConvertNumber(vObject, out vConvertDecimal);
                    if (IsConvert == true)
                    {
                        vConvertString = string.Format("{0:###,###,###,###,###,###,###,###}", vConvertDecimal);
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid_Sum, InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, string pPeriod)
        {// 실제 호출되는 부분.
        ...
        private void IsNewPage(int pPageRowCount)
        {
            ...
            { // pPrintingLine : 현재 출력된 행.
                ...
                mPrinting.XLHPageBreaks_Add(mPrinting.XLGetRange("A" + (mCurrentRow + iDefaultEndRow)));
